package bei7473p5254d69jcuat.tenyutalk.ui.reference;

import bei7473p5254d69jcuat.tenyutalk.model.release1.*;
import bei7473p5254d69jcuat.tenyutalk.reference.*;

public class TenyutalkReferenceSecureGui<V extends CreativeObject>
		extends TenyutalkReferenceBaseGui<V> {

	public TenyutalkReferenceSecureGui(String name, String cssIdPrefix) {
		super(name, cssIdPrefix);
		// TODO 自動生成されたコンストラクター・スタブ
	}

	@Override
	public void set(TenyutalkReferenceBase<V> o) {
		// TODO 自動生成されたメソッド・スタブ
		super.set(o);
	}


}
