...
import bei7473p5254d69jcuat.tenyu.release1.communication.request.HasFile.*;
...
public class Downloader implements GlbMemberDynamicState {
	...
	public boolean downloadSync(TenyuFile f, boolean necessary) {
		try {
			...
			Glb.getLogger().info("start downloading file=" + f);
	...
	private boolean downloadSyncInternal(TenyuFile f, boolean necessary) {
		//このあたりでビットと言っているのはたいていwriteBitであり1ビットあたり
		...
		if (successCount == 0) {
			Glb.getLogger().warn("no node of file holder");
			return false;
		}
	 ...
	 *
	 * 未DLの部分は断片化している可能性がある。
	 *
	 * できるだけ新たな断片化を抑えて、まとまった単位でリクエストしたい。
	 * 処理効率が上がる。
	 *
	 * できるだけ低レイテンシノードにより大きな負荷を担わせたい。
	 * ネットワーク全体を想像した時、地理的に近いところに問い合わせるべきである。
	 *
	 * @return	DLに成功したか
	 */
	private boolean downloadSyncInternalFragmentation(Ctx ctx) {
		//低レイテンシノードはアクセスが増える一方でストレージ性能の高さは不明だから、
		//できるだけ連続アクセスにすべき。
		//高レイテンシノードに断片化した部分を担当させる。
		//断片化はCPUやストレージの負荷を高める一方で通信量の増加は限定的

		//荷物一覧。担当が決まったら随時削除される。荷物の分割による追加もある
	...
	private <I extends IdObjectDBI, O extends I> void periodicDownloadCommon(
		...
		local.id = IdObjectDBI.getFirstRecycleId();
		int dbLoop = 1000;
		long sleep = 200;
		while (true) {
			try {
	 ...
	 * @return
	 */
	private boolean requestAndWrite(Ctx ctx) {
		AsyncRequestStatesNoRetry<
