...
public class P2P implements GlbMemberDynamicState {
	...
	private Message buildGuiRequest(String title, String content, P2PEdge to) {
		...
		content = me.getRecycleId() + " " + me.getName() + " : " + content;
	...
	public RequestFutureUser requestAsync(Message m,
		...
		if (resState == null) {
			...
			connectionFailed(receiver.getRecycleId());
		} else {
			connectionSuccess(receiver.getRecycleId());
		}
	...
	public Message requestUserRightMessage(
		...
		return requestToServer(to -> {
			...
			if (!(req instanceof Content)) {
				Glb.getLogger().error("invalid Request object",
						new Exception());
				return null;
			}
			return Message.build((Content) req).packaging(req.createPackage())
					.finish();
	...
	public boolean sendAsync(Message m, NodeIdentifierUser identifier) {
		...
		if (r) {
			connectionSuccess(u.getRecycleId());
		} else {
			connectionFailed(u.getRecycleId());
		}
		return r;
	}
	...
	public static class RequestHandler
		...
		public Response getRes() {
			...
			if (req == null || req.getRes() == null)
				return null;
			Content c = req.getRes().getContent();
