...
*
*/
public class P2PDefense extends IdObject implements GlbMemberDynamicState {
	...
	public static P2PDefense loadOrCreate() {
		return Glb.getDb(Glb.getFile().getDefenseDBPath())
				.computeInTransaction((txn) -> {
					P2PDefense r = null;
					try {
						r = new P2PDefenseStore(txn).get();
					} catch (Exception e) {
	...
	public boolean isOverSize(Long userId, Message received) {
		...
		//作者か
		boolean author = Glb.getConst().getAuthor().getRecycleId()
				.equals(userId);
