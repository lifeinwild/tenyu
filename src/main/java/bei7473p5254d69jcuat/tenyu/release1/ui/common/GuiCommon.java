...
public class GuiCommon {
	 ...
	 * @return
	 */
	public static int buildObjectivityObject(GuiBuilder builder, GridPane grid,
			int elapsed, CRUDContext ctx, ObjectivityObjectGui built) {
	...
	public static List<UploadFileGui> getFilesFromDir(Path start,
		...
		try {
			Files.walk(start).filter(Files::isRegularFile).forEach(arg -> {
				...
				UploadFile.validateRelativeFilePathStatic(relativePath, vr);
				if (!vr.isNoError()) {
					gui.message(vr);
					return;
				}
