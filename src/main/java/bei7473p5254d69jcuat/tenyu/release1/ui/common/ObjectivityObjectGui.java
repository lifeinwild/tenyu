...
public abstract class ObjectivityObjectGui<T1 extends ObjectivityObjectDBI,
		T2 extends T1,
		T3 extends ObjectivityObject,
		S extends ObjectivityObjectStore<T1, T2>,
		G extends ObjectivityObjectGui,
		TI extends ObjectivityObjectTableItem<T1, T2>>
		extends IdObjectGui<T1, T2, T3, S, G, TI> {

	public ObjectivityObjectGui(String name, String id) {
		super(name, id);
	}
	...
	public void buildSearchResultObjectivityObject() {
	...
	/**
	 * ObjectivityObjectに関するイベントハンドラを登録
	 * @param getStoreFunc
	 * @param clearFunc
	 * @param built
	 * @param singleFunc
	...
	private void build() {
		...
		registererLabel = new Label(
				Lang.OBJECTIVITYOBJECT_REGISTERER_NAME.toString());
		...
		mainAdministratorNameLabel = new Label(
				Lang.OBJECTIVITYOBJECT_ADMINISTRATOR_NAME.toString());
	...
	public void setGui(ObjectivityObjectGui<T1, T2, T3, S, G, TI> gui) {
		super.setGui(gui);
