...
import bei7473p5254d69jcuat.tenyu.release1.ui.creator.game.*;
import bei7473p5254d69jcuat.tenyu.release1.ui.creator.game.ratinggame.RatingGameGui.*;
...
public class RatingGameGui extends
	...
	public static class RatingGameTableItem
			extends AbstractGameTableItem<RatingGameDBI, RatingGame> {
	...
	private boolean addClientFileButtonValidation(SubmitButtonGui gui) {
		try {
			...
			Naturality.validateName(clientFilesDir.toString(), vr,
					UploadFile.nameMax);
			if (!vr.isNoError()) {
				gui.message(vr);
				return false;
	...
	public TableView<GameClientFileTableItem> getFilesTable() {
		return filesTable;
	}

	public void setFilesTable(TableView<GameClientFileTableItem> filesTable) {
