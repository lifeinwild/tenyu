...
public class UserGui extends
		NaturalityGui<UserDBI, User, User, UserStore, UserGui, UserTableItem> {
	...
	//アバター設定
	private Label avatarCfgLabel;

	private TableView<AvatarConfigItemGui> avatarConfigTable;
	//FQDN
	...
	private void buildAvatar() {
		//アバター
		VBox avatarPane = new VBox();
		avatarPane.setId(idPrefix + "AvatarPane");
		avatarPane.setPadding(new Insets(25, 25, 25, 25));
		avatarPane.setFocusTraversable(true);
		int avatarPaneElapsed = 0;

		if (ctx == CRUDContext.UPDATE) {
			//アバター追加GUI
			AvatarGui avatar = new AvatarGui(Lang.USER_AVATAR.toString(),
					idPrefix + "AvatarConfig");

			SubmitButtonFuncs sbf = new SubmitButtonFuncs(
					Lang.USER_AVATAR_CONFIG_ADD.toString(),
					idPrefix + "AvatarConfigAdd", gui -> {
						String s = avatar.getNameInput().getText();
						if (s == null || s.length() == 0) {
							gui.message(Lang.NATURALITY_NAME_EMPTY.toString());
							return false;
						}
						Naturality o = Glb.getObje()
								.getAvatar(as -> as.getByName(s));
						if (o == null || !(o instanceof Avatar)) {
							gui.message(Lang.NOT_FOUND.toString());
							return false;
						}
						Avatar a = (Avatar) o;
						int i = getAvatarConfigTable().getItems().size();
						//通信ではないのでここで処理する
						getAvatarConfigTable().getItems()
								.add(new AvatarConfigItemGui(
										new AvatarConfigItem(a.getRecycleId(),
												i)));
						return true;
					}, null, null, null);
			avatar.buildSearchSimple(null);
			avatar.buildExternalButton(sbf);
			add(avatar);
		}

		setAvatarCfgLabel(new Label(Lang.AVATAR.toString()));
		grid.add(getAvatarCfgLabel(), 0, elapsed);
		setAvatarConfigTable(new TableView<>());
		getAvatarConfigTable().setId(idPrefix + "AvatarConfigTable");
		getAvatarConfigTable().setEditable(false);
		getAvatarConfigTable().setMinWidth(240);
		TableColumn<AvatarConfigItemGui,
				Integer> priorityHead = new TableColumn<>(
						Lang.PRIORITY.toString());
		priorityHead.setCellValueFactory(
				new PropertyValueFactory<AvatarConfigItemGui, Integer>(
						"priority"));
		TableColumn<AvatarConfigItemGui,
				String> avatarNameHead = new TableColumn<>(
						Lang.AVATAR.toString());
		avatarNameHead.setCellValueFactory(
				new PropertyValueFactory<AvatarConfigItemGui, String>(
						"avatarName"));
		getAvatarConfigTable().getColumns().addAll(priorityHead,
				avatarNameHead);
		avatarPane.getChildren().add(getAvatarConfigTable());
		avatarPaneElapsed += 4;
		if (ctx == CRUDContext.UPDATE) {
			//アバター下部ボタン類
			GuiCommon.buildSubmitButton(avatarPane, avatarPaneElapsed,
					Lang.USER_AVATAR_CONFIG_UP.toString(), idPrefix, gui -> {
						AvatarConfigItemGui selected = getAvatarConfigTable()
								.getSelectionModel().getSelectedItem();
						if (selected == null) {
							gui.message(Lang.CHOICE_ONE.toString());
							return false;
						}
						List<AvatarConfigItemGui> items = getAvatarConfigTable()
								.getItems();
						if (items == null || items.size() == 0) {
							gui.message(Lang.EXCEPTION.toString());
							return false;
						}
						int selectedIndex = getAvatarConfigTable()
								.getSelectionModel().getSelectedIndex();
						int newIndex = selectedIndex - 1;
						if (newIndex >= 0) {
							Collections.swap(items, selectedIndex, newIndex);
						} else {
							gui.message(Lang.USER_AVATAR_CONFIG_OUT_OF_RANGE
									.toString());
						}
						return true;
					}, null, null, null);
			avatarPaneElapsed += 1;
			GuiCommon.buildSubmitButton(avatarPane, avatarPaneElapsed,
					Lang.USER_AVATAR_CONFIG_DOWN.toString(), idPrefix, gui -> {
						AvatarConfigItemGui selected = getAvatarConfigTable()
								.getSelectionModel().getSelectedItem();
						if (selected == null) {
							gui.message(Lang.CHOICE_ONE.toString());
			...
			GuiCommon.buildSubmitButton(avatarPane, avatarPaneElapsed,
					Lang.USER_AVATAR_CONFIG_DELETE.toString(), idPrefix,
					gui -> {
						AvatarConfigItemGui selected = getAvatarConfigTable()
								.getSelectionModel().getSelectedItem();
						if (selected == null) {
						...
						return items.remove(selected);
					}, null, null, null);
			avatarPaneElapsed += 1;
		}
		grid.add(avatarPane, 1, elapsed);
		elapsed += avatarPaneElapsed;
	}
	...
	private void buildNaturalityPrompt() {
		//説明のプロンプト
		getExplanationInput()
	...
	public User setupModelUpdateOrDelete(User exist) {
		...
		exist.getAvatarConfig().clear();
		for (int i = 0; i < avatarConfigTable.getItems().size(); i++) {
			AvatarConfigItemGui e = avatarConfigTable.getItems().get(i);
			exist.getAvatarConfig().add(e.getSrc().getAvatar().getRecycleId(),
					i);
		}
	...
	public void set(User u) {
		...
		if (avatarConfigTable != null)
			Glb.getGui().updateTable(u.getAvatarConfig().getAvatarConfigItems(),
					(src) -> new AvatarConfigItemGui(src), avatarConfigTable);
	}
	...
	public Label getAvatarCfgLabel() {
		return avatarCfgLabel;
	}

	public void setAvatarCfgLabel(Label avatarCfgLabel) {
		this.avatarCfgLabel = avatarCfgLabel;
	}

	public TableView<AvatarConfigItemGui> getAvatarConfigTable() {
		return avatarConfigTable;
	}

	public void setAvatarConfigTable(
			TableView<AvatarConfigItemGui> avatarConfigTable) {
		this.avatarConfigTable = avatarConfigTable;
	}
