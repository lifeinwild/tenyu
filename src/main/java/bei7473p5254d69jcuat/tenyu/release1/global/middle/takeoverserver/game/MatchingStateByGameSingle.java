...
public class MatchingStateByGameSingle extends AbstractMatchingStateByGame {
	 ...
	 * @param gref		gameと対応するgref
	 * @param teamName	チーム名
	 * @return			作成されたチーム
	 */
	protected Team createTeam(RatingGame game, GameReference gref,
			String teamName) {
		Team t = new Team(gref, teamName, null,
	...
	private Teams createTeams(List<NodeIdentifierUser> applicants,
		...
		for (NodeIdentifierUser applicant : applicants) {
			...
			if (t == null || t.isFull()) {
				...
				t = createTeam(game, gameRef, teamName);
			}
			//メンバーを加える
	...
	public List<RatingGameMatch> matching() {
		...
		game = (RatingGame) gameRef.getGame();
