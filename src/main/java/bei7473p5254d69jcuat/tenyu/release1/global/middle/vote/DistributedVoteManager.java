package bei7473p5254d69jcuat.tenyu.release1.global.middle.vote;
	...
	public boolean put(DistributedVote vote) {
		...
		return Glb.getObje().compute(txn -> {
			try {
				...
				boolean r = false;
				if (s.get(vote.getRecycleId()) != null) {
					//存在するなら更新
					r = s.update(vote);
				} else {
					//存在しないなら作成
					if (vote.getRecycleId() == null) {
						r = s.create(vote) != null;
					} else {
						r = s.createSpecifiedId(vote) != null;
					}
				}
				return r;
			} catch (Exception e) {
