...
public class Team implements Storable {
	public static final int nameMax = Naturality.nameMax + 50;
	...
	public static byte[] passwordToHash(String password) {
		if (password == null)
			return null;
		return Glb.getUtil()
				.hash(password.getBytes(Glb.getConst().getCharsetNio()));
	}
	...
	public Team(GameReference gref, String teamName, String password,
	...
	public synchronized boolean addMember(NodeIdentifierUser node,
		...
		boolean dbCheck = Glb.getObje().readRet(txn -> {
			try {
				if (gameRef.getGameId() == null
						|| gameRef.getType() != GameType.RATINGGAME)
					return false;
				RatingGameStore rgs = new RatingGameStore(txn);
				RatingGame game = rgs.get(gameRef.getGameId());
				if (game == null)
					return false;
				...
				if (sos.isBlock(NodeType.RATINGGAME, game.getRecycleId(),
						node.getUserId()))
					return false;
			...
			try {
				RatingGameStateByUser stateByUser = Glb.getObje()
						.getRatingGameStateByUser(
								rgsbus -> rgsbus.getByGameIdUserId(
										gameRef.getGameId(), node.getUserId()));
				...
				Glb.getLogger().error(
						"userId=" + node + " gameId=" + gameRef.getGameId(), e);
				continue;
			}
		}
	...
	public RatingGame getGame() {
		Object g = gameRef.getGame();
		if (g == null || !(g instanceof RatingGame)) {
			return null;
		}
		return (RatingGame) g;
	}
	...
	public String toString() {
		...
		try {
			gameName = gameRef.getGame().getName();
		} catch (Exception e) {
			Glb.getLogger().error("", e);
		}
		TeamReference tref = getReference();
		return tref.toString() + " state=" + state.name() + " gameName="
				+ gameName + " admin=" + admin + " members=" + members;
	}
	...
	private boolean validateAtCommon(ValidationResult vr) {
		...
		if (gameRef == null) {
			vr.add(Lang.GAMEPLAY_MATCHING_TEAM_GAMEREF, Lang.ERROR_EMPTY);
			b = false;
		}
		if (teamClassId == -1) {
		...
		if (name == null || name.length() == 0) {
			...
			if (!Naturality.validateName(Lang.GAMEPLAY_MATCHING_TEAM_NAME, name,
					vr, nameMax)) {
				b = false;
			}
		}
	...
	public boolean validateReference(ValidationResult r, Transaction txn)
		...
		if (gameRef != null) {
			if (!gameRef.validateReference(r, txn)) {
				...
				RatingGame rg = (RatingGame) gameRef.getGame();
				TeamClass tc = rg.getTeamClass(teamClassId);
				if (tc == null) {
