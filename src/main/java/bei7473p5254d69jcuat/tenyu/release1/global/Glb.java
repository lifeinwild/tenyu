...
import bei7473p5254d69jcuat.tenyu.release1.*;
import bei7473p5254d69jcuat.tenyu.release1.communication.*;
import bei7473p5254d69jcuat.tenyu.release1.communication.Content.*;
import bei7473p5254d69jcuat.tenyu.release1.communication.local.*;
...
import bei7473p5254d69jcuat.tenyu.release1.communication.mutual.vote.*;
...
import bei7473p5254d69jcuat.tenyu.release1.communication.request.gui.GuiCausedSimpleMessageGui.*;
...
import bei7473p5254d69jcuat.tenyu.release1.communication.request.server.ratinggamematchingserver.*;
import bei7473p5254d69jcuat.tenyu.release1.communication.request.subjectivity.*;
import bei7473p5254d69jcuat.tenyu.release1.communication.request.subjectivity.CommonKeyExchange.*;
import bei7473p5254d69jcuat.tenyu.release1.communication.request.subjectivity.CommonKeyExchangeConfirmation.*;
...
import bei7473p5254d69jcuat.tenyu.release1.communication.request.subjectivity.Recognition.*;
...
import bei7473p5254d69jcuat.tenyu.release1.global.middle.takeoverserver.game.Team.*;
...
import bei7473p5254d69jcuat.tenyu.release1.global.timer.*;
 ...
 * {@link Objectivity#applyMessageList(UserMessageList)}でその保証が壊れている。
 * オンメモリのオブジェクトに関してトランザクションとかMVCCが無いから。
 *
...
public class Glb {
	 ...
	 *
	 */
	private transient static String password;//TODO アップデートモジュールに移動
	 ...
	 * 相互評価フローネットワークのフロー計算
	 */
	private static FlowComputation flow;
	...
	private static void kryoSetupForContent(Kryo k) {
		...
		k.register(GetRecycleIDList.class);
		k.register(GetRecycleIDListResponse.class);

		k.register(RegisterRatingMatches.class);
		k.register(RatingGameMatch.class);
		k.register(Team.class);
		k.register(GameReference.class);
	...
	public static void setupStores() {
		...
		getObje().execute(txn -> {
			for (String storeName : obje.getIdObjectStoreNames()) {
				IdObjectStore<?, ?> s = Glb.getObje().getStore(storeName, txn);
				s.initStores();
				try {
					RecycleIdStore recycle = new RecycleIdStore(storeName, txn);
					recycle.initStores();
	...
	public static FlowComputation getFlow() {
		return flow;
	}
	...
	public static void setFlow(FlowComputation flow) {
		Glb.flow = flow;
	}
