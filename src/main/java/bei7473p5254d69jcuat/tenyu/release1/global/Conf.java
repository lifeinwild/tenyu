...
public class Conf {
	 ...
	 *
	 * ※もともとHDDを想定していたが、現在HDDは想定せずSSDを想定する。
	 * 予想よりファイルサイズを抑えれそうである事と、
	 * PCはもはやHDDを内蔵しない。
	 *
	 * Tenyu基盤ソフトウェアが書き込み操作をするフォルダは
	 * 必ずtenyuをパスに含む必要がある。
	 * だから接頭辞にtenyuが入っている。
	 *
	 ...
	 */
	private String dynamicFilesDir = "tenyuDynamicFiles/";
	...
	/**
	 * 鍵をセットアップするが、鍵の作成や読み込みはパスワードを必要とする。
	 ...
	 * @param password
	 */
	public void setupKeys(String password) {
		try {
			//鍵が未作成なら
			if (!isKeyGenerated()) {
				String date = getDate();
	 ...
	 * @return			鍵ペア
	 */
	private KeyPair load(KeyType type, String password) {
		try {
	...
	private byte[] loadPri(KeyType type, String password) throws Exception {
