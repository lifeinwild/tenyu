 ...
 * ByteIterableからbyte[]はgetBytesUnsafe()でできる。
 *
...
public class DBUtil {
	 ...
	 * ArrayByteIterable#getBytesUnsafe()実装はその問題が無い。
	...
	/**
	 * 注意点としてcnvS()は末尾に0を付加して前方一致検索を狂わせるので、getBytesしてからcnvBA推奨。
	 * @param sInfo
	 * @param key
	 * @param cnvKey
	 * @param cnvValue
	 ...
	 */
	/*	使っていないのでコメントアウト
	public <K, V> KVSRecord<K, V> prefixSingle(StoreInfo sInfo,
		...
		try (Cursor c = s.openCursor(txn)) {
			...
			//byte[]の前方一致検索にする処理
			byte[] here = c.getKey().getBytesUnsafe();
			if (here == null || here.length < key.getLength())
				return null;

			byte[] search = new byte[key.getLength()];
			System.arraycopy(key.getBytesUnsafe(), 0, search, 0, search.length);

			byte[] herePrefix = new byte[search.length];
			System.arraycopy(here, 0, herePrefix, 0, search.length);

			if (!Arrays.equals(search, herePrefix)) {
				return null;
			}
