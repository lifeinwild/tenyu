...
public class URLProvementRegexStore
		extends NaturalityStore<URLProvementRegexDBI, URLProvementRegex> {
	...
	@Override
	protected boolean createNaturalityConcrete(URLProvementRegexDBI o)
			throws Exception {
		for (String fqdn : o.getFqdns()) {
			if (!util.put(fqdnToId, cnvS(fqdn), cnvL(o.getRecycleId()))) {
				return false;
			}
		}
		return true;
	...
	protected boolean dbValidateAtUpdateNaturalityConcrete(
			...
			ValidationResult r) {
		boolean b = true;
		List<String> added = Glb.getUtil().getExtra(updated.getFqdns(),
				old.getFqdns());
		for (String fqdn : added) {
			if (existByFqdn(fqdn, updated.getRecycleId())) {
	...
	@Override
	protected boolean deleteNaturalityConcrete(URLProvementRegexDBI o)
			throws Exception {
		for (String fqdn : o.getFqdns()) {
	...
	@Override
	protected boolean existNaturalityConcrete(URLProvementRegexDBI o,
			ValidationResult vr) throws Exception {
		boolean b = true;
		for (String fqdn : o.getFqdns()) {
			if (!existByFqdn(fqdn, o.getRecycleId())) {
				vr.add(Lang.URLPROVEMENT_REGEX_FQDN, Lang.ERROR_DB_NOTFOUND,
						"fqdn=" + fqdn + " id=" + o.getRecycleId());
				b = false;
				break;
			}
		}
	...
	@Override
	protected boolean noExistNaturalityConcrete(URLProvementRegexDBI o,
			ValidationResult vr) throws Exception {
		boolean b = true;
		for (String fqdn : o.getFqdns()) {
			if (existByFqdn(fqdn, o.getRecycleId())) {
	...
	@Override
	protected boolean updateNaturalityConcrete(URLProvementRegexDBI updated,
