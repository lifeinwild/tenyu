...
public class EdgeLog extends AdministratedObject implements EdgeLogI {
	...
	private final boolean validateAtCommon(ValidationResult r) {
		...
		if (comment != null) {
			if (comment.length() > commentMax) {
				...
				if (!IndividualityObject.validateText(Lang.SOCIALITY_EDGELOG_COMMENT,
						comment, r)) {
					b = false;
				}
			}
	...
	@Override
	public EdgeLogGui getGuiReferenced(String guiName, String cssIdPrefix) {
		return new EdgeLogGui(guiName, cssIdPrefix);
	}

	@Override
	public EdgeLogStore getStore(Transaction txn) {
		return new EdgeLogStore(txn);
	}

	@Override
	public StoreNameEnum getStoreName() {
		return StoreNameObjectivity.EDGE_LOG;
	}
