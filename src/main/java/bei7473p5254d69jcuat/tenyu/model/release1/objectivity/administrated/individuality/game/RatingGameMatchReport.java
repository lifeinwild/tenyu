 ...
 *
 */
public class RatingGameMatchReport implements ValidatableI {
	...
	public byte[] getSignTarget() {
		...
		sign = null;
		try {
			return Glb.getUtil().hashSecure(
					Glb.getUtil().toKryoBytesForCommunication(this));
		} catch (IOException e) {
			Glb.getLogger().error("", e);
		} finally {
			sign = signBackup;
	...
	public byte[] sign() {
		try {
			return Glb.getConf().getKeys().sign(getNominal(), keyType,
					getSignTarget());
