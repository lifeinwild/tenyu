package bei7473p5254d69jcuat.tenyu.model.release1.objectivity.administrated.individuality.dtradable;
...
public class DistributedTradable extends IndividualityObject
	...
	public static boolean verify(DistributedTradable o, Long latestOwner,
		...
		for (; start < log.size(); start++) {
			...
			byte[] target = generateSignTarget(o.getId(),
					currentOwner.getId(), trade.getNextOwnerUserId());
	...
	@Override
	public DistributedTradableStore getStore(Transaction txn) {
		return new DistributedTradableStore(txn);
	}
	...
	public StoreNameEnum getStoreName() {
		return StoreNameObjectivity.DISTRIBUTED_TRADABLE;
	}
