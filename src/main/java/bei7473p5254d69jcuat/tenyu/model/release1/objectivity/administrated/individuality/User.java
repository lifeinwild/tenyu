...
public class User extends IndividualityObject implements ChainVersionup, UserI {
	...
	public static final int modelConditionIdsMax = 2000;
	 ...
	 * @return	セキュア鍵か
	 */
	public static boolean isSecureKeyLen(int keyLen) {
		int normalLenBits = getRsaKeySizeBitBySecure(false);
		if (keyLen == normalLenBits)
			return false;
		int secLenBits = getRsaKeySizeBitBySecure(true);
		if (keyLen == secLenBits)
			return true;
	...
	/**
	 * 鍵を更新した場合に古い鍵を記録しておくところ
	 */
	private KeysLog keysLog = new KeysLog();
	...
	/**
	 * このユーザーが受け取る通知の条件
	 * {@link Tenyupedia}に通知処理がある
	 */
	private List<Long> modelConditionIds = new ArrayList<>();
	...
	private Map<GraphicStyle, Avatar> styleToAvatar = new HashMap<>();
	...
	public boolean equals(Object obj) {
		...
		if (!Arrays.equals(mobilePublicKey, other.mobilePublicKey))
			return false;
		if (modelConditionIds == null) {
			if (other.modelConditionIds != null)
				return false;
		} else if (!modelConditionIds.equals(other.modelConditionIds))
			return false;
		if (nodeNumberToAddr == null) {
	...
	@Override
	public UserGui getGuiReferenced(String guiName, String cssIdPrefix) {
	...
	public List<Long> getModelConditionIds() {
		return modelConditionIds;
	}
	...
	public int hashCode() {
		...
		result = prime * result + ((modelConditionIds == null) ? 0
				: modelConditionIds.hashCode());
	...
	public void setKeysLog(KeysLog keysLog) {
		this.keysLog = keysLog;
	}
	...
	public void setModelConditionIds(List<Long> modelConditionIds) {
		this.modelConditionIds = modelConditionIds;
	}
	...
	public String toString() {
		return "User [nodeNumberToAddr=" + nodeNumberToAddr
				+ ", modelConditionIds=" + modelConditionIds
	...
	protected final boolean validateAtCreateIndividualityObjectConcrete(
		...
		if (!keysLog.validateAtCreate(r)) {
			b = false;
		}
		for (Avatar e : styleToAvatar.values()) {
	...
	protected boolean validateAtUpdateChangeIndividualityObjectConcrete(
		...
		if (!keysLog.validateAtUpdateChange(r, old2.getKeysLog())) {
			b = false;
		}

		for (Entry<GraphicStyle, Avatar> e : styleToAvatar.entrySet()) {
			if (!e.getValue().validateAtUpdateChange(r,
					old2.getStyleToAvatar().get(e.getKey()))) {
				b = false;
				break;
			}
		}
	...
	public Map<GraphicStyle, Avatar> getStyleToAvatar() {
		return styleToAvatar;
	}

	public void setStyleToAvatar(Map<GraphicStyle, Avatar> styleToAvatar) {
		this.styleToAvatar = styleToAvatar;
	}
	...
	private final boolean validateCommon(ValidationResult r) {
		...
		try {
			...
			if (getMobilePublicKey() == null
				...
				try {
					if (!u.isValidRSAKey(getMobilePublicKey(), rsaKeyLen)) {
			...
			if (getPcPublicKey() == null || getPcPublicKey().length == 0) {
				...
				try {
					if (!u.isValidRSAKey(getPcPublicKey(), rsaKeyLen)) {
			...
			if (getOfflinePublicKey() == null) {
				...
				try {
					if (!u.isValidRSAKey(getOfflinePublicKey(), rsaKeyLen)) {
			...
			if (modelConditionIds == null) {
				r.add(Lang.USER, Lang.MODEL_CONDITION_IDS, Lang.ERROR_EMPTY);
				b = false;
			} else {
				if (modelConditionIds.size() > modelConditionIdsMax) {
					r.add(Lang.USER, Lang.MODEL_CONDITION_IDS,
							Lang.ERROR_TOO_MANY,
							"size=" + modelConditionIds.size());
					b = false;
				} else {
					if (!Model.validateIdStandard(modelConditionIds)) {
						r.add(Lang.USER, Lang.MODEL_CONDITION_IDS,
								Lang.ERROR_INVALID);
						b = false;
					}
	...
	public boolean validateReferenceIndividualityObjectConcrete(
		...
		for (Long mcId : modelConditionIds) {
			if (mcs.get(mcId) == null) {
				r.add(Lang.USER, Lang.MODEL_CONDITION_ID,
						Lang.ERROR_DB_NOTFOUND_REFERENCE, "mcId=" + mcId);
				b = false;
			}
		}

		if (!keysLog.validateReference(r, txn))
			b = false;
		for (Avatar e : styleToAvatar.values()) {
	...
	/**
	 * デフォルトの{@link GraphicStyle#FREE}で設定する。
	 * @param a	アバター
	 * @return
	 */
	public boolean addAvatar(Avatar a) {
		return addAvatar(GraphicStyle.FREE, a);
	}

	public boolean addAvatar(GraphicStyle gs, Avatar a) {
		styleToAvatar.put(gs, a);
		return true;
	}
