...
public abstract class AdministratedObjectStore<
		...
		T2 extends T1> extends ModelStore<T1, T2> {
	...
	@Override
	protected final boolean createModelConcrete(T1 o) throws Exception {
	...
	@Override
	protected final boolean dbValidateAtUpdateModelConcrete(T1 updated,
	...
	@Override
	protected final boolean deleteModelConcrete(T1 o) throws Exception {
	 ...
	 * これをpublicにして直接呼び出す。
	 * @see bei7473p5254d69jcuat.tenyu.db.store.ModelStore#existModelConcrete(bei7473p5254d69jcuat.tenyu.db.store.ModelI, glb.util.ValidationResult)
	 */
	@Override
	...
	@Override
	protected List<StoreInfo> getStoresModelConcrete() {
	...
	/* (非 Javadoc)
	 * @see bei7473p5254d69jcuat.tenyu.db.store.ModelStore#noExistModelConcrete(bei7473p5254d69jcuat.tenyu.db.store.ModelI, glb.util.ValidationResult)
	 * GUI等においてDBとの整合性を検証するとき、
	 * まだIDが割り振られていないのでexist()を呼べないので、
	 * これをpublicにして直接呼び出す。
	 */
	@Override
	public final boolean noExistModelConcrete(T1 o, ValidationResult vr)
			throws Exception {
		boolean b = true;
		if (existByRegisterer(o.getRegistererUserId(), o.getId())) {
	...
	@Override
	protected final boolean updateModelConcrete(T1 updated, T1 old)
			throws Exception {
		if (Glb.getUtil().notEqual(updated.getRegistererUserId(),
				old.getRegistererUserId())) {
