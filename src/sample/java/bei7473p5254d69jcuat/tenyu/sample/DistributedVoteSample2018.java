...
/**
 * このコードは分散合意の性質を分かり易く示すためのもの。
 * 色々単純化している。
 *
 * @author exceptiontenyu@gmail.com
 *
 */
public class DistributedVoteSample2018 {
	...
	private static Long synctest() {
		...
		//第三	不正値かつ悪意ある改造ﾌﾟﾛｸﾞﾗﾑのﾉｰﾄﾞ	20%	悪意から正常値に復帰しない
		//この条件下で第二ｸﾞﾙｰﾌﾟが第一ｸﾞﾙｰﾌﾟへと復帰した事は騙されていたﾉｰﾄﾞが正常なﾉｰﾄﾞへと回復した事を意味し､
		//同調処理によって定期的にこのような処理が行われるから、
		//短時間の間に45%を超えるﾉｰﾄﾞを不正値にする必要がある事を意味し､改ざんを成功させるのが困難になる｡

		// P2Pﾈｯﾄﾜｰｸで全ﾉｰﾄﾞの情報を集計して多数決する事は､やったとしても
		// その集計ﾉｰﾄﾞを信用できないので意味が無い｡
		// ところが、分散合意は近傍さえ信用できるならその収束値を全ノード共通の値として採用できる｡
		// ここで1つ常識が覆されている。中央機関無し、集計無しでの合意が実現されている。
		// これだけのアイデアではダミーノードに弱いと思うかもしれないが、
		// ﾌﾟﾛｾｯｻ証明が近傍に信用を与え、裏を返せばダミーノードに排除圧を与える。
