...
## 全体的なイメージ
各ノードは普段近傍と客観DBについて**各々のタイミングで随時**同調処理しています。(この動作は分散合意のサンプルコードと比べるとターン概念が無く局所的多数決のタイミングが自由であるという点に注意)
...
ここは2種類の実装方法があり、現在のネットがブロードキャストの方法を提供していないので、ブロードキャストに頼らない方法を採用しています。

一斉更新の時同調処理は短時間停止しています。
