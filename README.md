...
[社会的認識の保全を義務ないし道徳として捉えれる可能性があります](#コモンウェルス)。
...
「実態に基づいた社会的認識を持てる社会」をある種の環境的財産と捉えれば、私が主張する[作者権](#作者権)等はコモンウェルスからも説明される](#なぜ社会を騙してはいけないか)。
...
- [全ノード一斉](#ターン)に[局所的多数決](#局所的多数決)をするのか、各々のタイミングでやるのか。

[選挙](#選挙)は開始時点の状態（誰にどの程度投票するか）がノード毎に当然にばらばらで、終了時点では平均値に収束（全ノードで一致）し、全ノード一斉に局所的多数決をします。

[同調処理](#同調処理)は開始時点の状態は６～９割程度のノードで一致していて（[客観DB](#客観)）、終了時点ではほぼ全ノードで一致し、各ノードばらばらのタイミングで同調します。
