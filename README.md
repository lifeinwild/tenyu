...
- **政治家がどう経済成長するのか環境問題をどうするのかなど議論している事がありますが**、Tenyuはそのような問題への[独自の回答](#何に価値を感じるべきか)を持ちます。財政問題を解消しうるほどの経済成長の可能性がありかつ環境負荷が限定的である経済構想です。ソフトウェアという新しい領域を適切なモデルで経済化し、[経済成長の軸をソフトウェアやMMOに移す事による環境負荷の軽減](#何に価値を感じるべきか)。
...
- **政治的主張**  
著作権やソフトウェア特許を[作者権](#作者権)に変えるべきとか、[環境負荷低減のために仮想経済を軸に経済成長しろ](#何に価値を感じるべきか)という政治的主張があります。
- **世界的利益**  
...
# 仮想経済
私が使用する仮想経済という語は私の造語です。

仮想経済とは[RMT市場](https://ja.wikipedia.org/wiki/%E3%83%AA%E3%82%A2%E3%83%AB%E3%83%9E%E3%83%8D%E3%83%BC%E3%83%88%E3%83%AC%E3%83%BC%E3%83%87%E3%82%A3%E3%83%B3%E3%82%B0)を創出しうる仮想的な経済です。典型的には[MMO](#MMOとは何か)ですが、そのRMT市場の創出力を強調したものです。

仮想経済をどうデザインすべきかは非常に難しい問題です。その問題領域がどんな枠組みの上にあるかというところから問題になります。私の考えでは、ゲームというただの娯楽の問題ではなく[ソフトウェアのための新たな資本モデル](#目的)を構想する上で本質的な問題です。私の構想では仮想経済は人々から[共同主体](#共同主体の重要性)へと仮想通貨を回収する役割を持ちます。

Tenyuは仮想経済の設計に対する独自の回答を持ちます。[連合型MMO](#連合型MMO)

# 連合型MMO
連合型MMOは多数の制作者がそれぞれ小さなゲームを登録して、それらを連携させて[MMO](#MMOとは何か)を実現します。  
...
## ギルド
当初ギルド概念は本構想にありませんでしたが見直しました。ギルドもMMOの醍醐味であり扱う必要があります。ギルドは**共同育成対象**であり、ギルドで挑むようなコンテンツがあればギルドメンバーの成長はどのギルドメンバーにとっても利益となります。互いに成長を助ける動機が存在します。
...
ノードは任意のタイミングでオフラインになったりオンラインになったりするので、固定的な連絡網を構築するというアイデアがあまり通用しません。連絡網というアイデアは、P2Pネットワークをメッセージが拡散していくときの経路が重複しないようにする面がある。任意のノードから生じるメッセージを拡散させなければならないというのも連絡網の設計を難しくします。通常の連絡網は情報の発生元を限定できます。
...
もし効率的なブロードキャスト、または世界的なサーバプロセス検索が伴ったマルチキャストの方法があればこの問題は解決されます。  
...
ブロードキャストではなく、世界中のサーバプロセスを任意の条件で検索してその検索結果を送信先とするマルチキャストが考えられる。
特定のP2Pネットワークの全ノードへの送信は、そのP2Pソフトウェアのサーバプロセスを検索してその検索結果をあて先一覧とする。  
その送信先への実際の送信処理は、地理的密集度等を考慮した無線と有線を使い分けた方法になるだろう。
サーバプロセスの検索は、サーバプロセスから提出された任意個数の特徴（文字列）をISP側システムがインデックスする事が考えられる。

インターネットがそのような基礎的機能を提供するとなれば、あるべきP2Pソフトウェアの設計は全く異なるものになる。

さらに、Tenyuの通信要件は近所のPCとの通信が多発する面があるので、各PC（またはルータ）が無線を使って直接近所のPCと通信できると効率がいい。
それについて考えると、自発的なインターネット網の構築という思想も見える。例えば各PCが無線機能を備えて近所のPCと相互に通信をリレーするなどしてインターネット網の補助ネットワークを構築するなど。

ちなみに、重複送信を防ごうとメッセージに送信済みノード一覧のような情報を記録するとメッセージのサイズがでかくなりすぎるので本末転倒となる。

他にグループネットワークみたいなものも考えた事がある。各ノードは同じグループの全ノードを知っていて誰かがメッセージを受け取ればグループ内全体に送信するとか。低レイテンシなノードでグループを作る、グループの全ノードが同時にオフラインにならないようにする、するとグループのネットワークが安定して連絡網を作れる、など。
とはいえこのアイデアもあまり良くなさそうだった。
グループの安定性の評価が難しいし、結局グループ内で重複受信が生じる。

あるいはAIで適応的に近傍関係を作り丈夫な拡散経路を作るか、などと考えた。

低レイテンシグループは地理的に近い位置に居る事を意味し障害で同時にオフラインになるリスクがある。すると連絡網がそこから先途絶する事になる。

さらに、インターネットが地上波や衛星波を駆使した広域送信機能を備えるべきではないか、PCが近所への無線送信能力を備えるべきではないか、というような**今後のあるべきインターネット網の発展**という事を考えると、グループネットワークのようなアイデアで解決を試みるのは筋が悪いように思う。
...
あるいはなぜ人々は仮想的なアイテムに価値を感じるべきか？
  ...
  - [仮想的なアイテムが価値を持つ](#仮想経済)と以下の理由から全体利益に適うので**それに価値があるという判断が行われる**。
    - 車や交通機関を利用する外出や、物品の消費を伴う趣味と比べて環境負荷を抑えれる。
    - ITの進歩を促進する可能性がある。
    ...
    - その入手や使用は自宅に居ながらできて手軽。
    - 自宅に居る時間も消費が発生し、しかも従来の商品と競合しない。
    - 副業として仮想的なアイテムの入手やその制作活動に参加できる。あるいはゲームプレイという娯楽の時間を収益化できる。
...
20世紀は工業製品の急速な発達があったが、そのような商品は可能な販売量に限界があるので経済成長に限界がある。環境負荷の問題もある。  
**21世紀はオンラインゲームのアイテムによる場所をとらず環境負荷も限定的な[上限の無い経済成長](#仮想経済)が実現される**。  
この観点はこの記事から影響を受けている。  
http://www5b.biglobe.ne.jp/~shu-sato/dc01.htm

この項目から外れるがネット上の経済は地方創生の希望だろうと思う。
