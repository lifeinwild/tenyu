...
しかしpure javaな良いゲームエンジンがありません。
jme3を考えていますがアセットの読み込みやリターゲッティング機能が弱く、可能なアセットツールチェーンが乏しすぎる。
現状、ゲームエンジンについて短期的には見通しが立っていません。
...
- ボーン名の違いに対処するには辞書的なデータを用意する必要がある。これはボーン探索プログラムによる構造の違いへの対処でも活用できる。
...
著作権や特許権は財産権に根拠づけられるようですが、作者権は何かに根拠づけられるものではなくそれ自体が独立した権利です。財産権は収益が望めないような創作的成果を保護できない可能性がありますが、作者権は収益性を成立要件とせず、誰が作者であるかという事実とその成果の創作性の高さを成立要件とします。
...
例えば、まとめサイトが掲示板のログを勝手にコピーしている事は悪質なパクリか？
...
他に[抽象ノード](#ノードとエッジの種類)やドネーションウェアのような収益化方法も考えられます。
...
それが正史の最終話であるという情報はデマであり、デマが広がる事はプロジェクト権で否定されます。
...
従来ソフトウェアはマネタイズの方法が確立していませんでした。ソフトウェア特許や著作権によるマネタイズは他者によるソフトウェアの自由な利用を阻害することを伴いました。相互評価フローネットワークはソフトウェアの[ほぼ自由な利用を推進しつつソフトウェアのほぼ汎用的なマネタイズを実現](#主旨)します。  
...
Tenyuは[プロセッサ証明](#プロセッサ証明)と[分散合意](#分散合意)という独自のP2P技術によって桁違いの性能と省電力をセキュリティを保ちつつ実現します。  
...
- 問題作成情報が近傍からの全ランダム値を含む事から、**近傍の近傍数をある種証明的に**知ることができます。全ランダム値の長さを１ランダム値の長さで割れば近傍数になります。これを過大に見せかける事はできますが過小に見せかける事はできません。もしそれをやると解くべき問題関数の数が増えるので不正に信用を獲得するという目的には適いません。これによって過剰近傍攻撃（極端に多くの近傍を持つ事によって分散合意を操作しようとする行為）が防止されます。
...
# 分散合意  
「P2Pネットワークの全ノードで共通の改竄困難な情報を持つにはどうすればいいか」という問題を「[局所的多数決](#局所的多数決)において[ダミーノード](#ダミーノード)を排除するにはどうすればいいか」という問題へ帰結させるアイデア。そして[プロセッサ証明](#プロセッサ証明)によってダミーノードが解決される。
...
# ネトゲはゲームコントローラーを前提とすべき
キーボード＋マウス操作は無理がある。従来良く用いられた操作方法は左手キーボードでスキル発動とWASDキャラ移動＋右手マウスでカメラ回転だった。この操作方法では左手の負担が大きく、自由にWASD移動しながら多様なスキルを発動するのはどうしても無理があった。ゲームコントローラーなら3Dスティックでキャラ移動とカメラ回転ができる。キーボードではWASDで指４本でキャラ移動を行っていたのに対して３Dスティックで親指１本でできる点でゲームコントローラーの方が優れている。一方でゲームコントローラーはボタンの数が少ないが、そこはゲームデザインが歩み寄り、キーストロークでスキル発動したりスキル数を限定したりモード（ある種のゲーム状態）を切り替えて１つのボタンに複数のスキルを対応付ける等考えられる。

ゲームコントローラーの欠点は持ち上げなければならないこと。キーボードやマウスは持ち上げる必要が無い。だから軽量化のため振動機能を無くすべき。

キーボードの長所はチャットがしやすいことだが、VCや音声認識が代替になるだろう。
