...
この段階では客観DBを全ノードで統一（同値）する方法が存在しないという事です。全ノードで統一された情報というものは基本的にありません。（基盤ソフトウェアによって与えられる定数等を除けば）

以下、各ノードが互いの信用値をどのように高めていくかという事、それでなぜ全体が堅牢になるか、どのように全体で共有される情報を作るかを説明します。

全体的な流れ。**問題作成情報作成＆送信→総合された問題作成情報作成→そこからハッシュ値を作成→ハッシュ値から問題関数を作成→引数探索→回答作成＆送信→回答検証＆信用値付与**。  

達成されている重要な性質。
- 1つの総合された問題作成情報に回答するだけで全近傍において信用量を獲得できます。これによって近傍数が増加しても引数探索（演算量証明）が増加しなくなります。
- 総合された問題作成情報が近傍からの全ランダム値を含む事から、出題者は自分が送信したランダム値に総合された問題作成情報が依存している事を確認できて今初めて確定した問題である事を確認できます。さらに、近傍の近傍数を知ることができてそれは過少報告が防止され、極端に多くの近傍を持つ事によって分散合意を操作しようとする行為が防止されます。
