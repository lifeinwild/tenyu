...
- 特殊な選挙的プロセスによる運営者選出
- 完全な透明性
完全な透明性は長所でも短所でもあり、例えばプログラミングを通じて共同資本から報酬を受け取るには基本的にソースコードを公開する必要がありますが、その制約はソースコードを秘匿すべきプロジェクトでは致命的となるので、つまり共同資本が扱える領域を限定します。共同資本はほとんどのソフトウェアをその構想上で扱えますが、完全に全てのソフトウェアを扱えるわけではありません。
