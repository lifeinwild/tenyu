...
TenyuのP2Pネットワークは定められた日時に選挙を実施して全体運営者を選出します。**選挙は従来技術で存在しなかったと思います**。つまりこの機能はいかにして高性能な客観DB更新をするかという話から**分離**された話で、達成される機能としては**別の話**です。しかし背景にある技術的アイデアはプロセッサ証明と分散合意であり同じです。ただ同調処理は各ノードのタイミングで随時行われるのに対して選挙は全ノードがタイミングを合わせて一斉に行います。選挙は高性能ではありませんが、極めて堅牢で、その意味から頻繁に行う必要があるものではありません。  
