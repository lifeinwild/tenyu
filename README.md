All rights reserved. 無断転用禁止。無断転載禁止。リンク可。記事やコメントやツイートにおけるURL付き引用可。

# Tenyuプロジェクトの概要
創作活動による経済を実現するためのプラットフォーム。独自のP2P技術による高性能な仮想通貨を含む。
P2Pベースのユーザー中心コンピューティングやゲームミドルウェア。
質問はissuesへ。

# 特徴、目的  
当面の目的は一通りのソフトウェアを作成する事と、創作活動をしている個人を集めてこの構想を回し始める事です。
具体的にはVtuberが参加できる状況を整備する。
私の構想している事が実現すれば、多くの人がソフトウェア制作やネット上での活動をライフワークとしてそれを収益化して暮らせます。

- 高性能な仮想通貨や改ざん困難なオンライン投票を実現する独自のP2P技術  
- 個人開発者が連携して連合型MMOを作り上げれるP2Pベースのゲームミドルウェア
- ソフトウェアを世界の共用資産とするためのライセンスと貢献度に応じて仮想通貨を獲得するシステム
- 様々なゲームで自分のアバターを使用する  
- 創作者の利益と創作の自由を両立する構想
- **オンラインゲームによって余暇を収益化したりオンラインゲームのプレイヤーが経済的理由でリアル破綻しないようにする**。本構想は成立すれば多数のオンラインゲームで共通のゲーム内通貨を用いる事になり一部ゲームが衰退したとしてもゲーム内通貨の価値が保たれるという面があります。プレイヤーは必要ならゲーム内通貨をRMTで販売できるでしょう。**ネットで金を稼ぎたい人達は大勢いると思いますが、他人から金を巻き上げるというアイデアは限界があります**。従来MMOがゲーム内通貨に価値をもたせRMT市場を実現していた事に注目してください。何も存在しないところに純粋にオンラインの活動だけで価値を作り出せる可能性があります。
- **ゲームの素材やプログラムや不正行為をしないユーザープール等を各ゲームで共有し簡単に高度なオンラインゲームを制作できるようにする**。相互評価フローネットワークによるソフトウェアのマネタイズや、それでマネタイズされる事を期待しソフトウェアを利用しやすいライセンスで公開するTenyuLicenseや、基盤ソフトウェアを通じた素材の共有システムやユーザーDBの提供等があります。
- **ゲーム関係に限らずネットで共有する価値が高いソフトウェアや知財を公開し、その背景にある複雑な貢献関係を明らかにし、作者に妥当な報酬を与える**。OSS抽象ノード等の概念で収益化されます。  
- **MAD動画の作者や配信者、フリーゲームの作者など小さなコンテンツのクリエイターが収益化される**。オンラインゲーム主軸モデルの衛星作品という概念で収益化されます。
...
- [Tenyuプロジェクトに影響を与えた創作物](#Tenyuプロジェクトに影響を与えた創作物)
- [必要スペック](#必要スペック)
- [推奨スペック](#推奨スペック)
- [基本概念](#基本概念)
  - [承認情報](#承認情報)
...
- [Tenyuの基礎技術の概要](#Tenyuの基礎技術の概要)
- [Tenyuの性能](#Tenyuの性能)
- [仮想通貨の流れ](#仮想通貨の流れ)
  - [相互評価フローネットワークを通じた仮想通貨分配](#相互評価フローネットワークを通じた仮想通貨分配)
  - [プレイヤー報酬](#プレイヤー報酬)
- [なぜP2Pで選挙的プロセスに基づくオンラインシステムを作らなければならないか？](#なぜP2Pで選挙的プロセスに基づくオンラインシステムを作らなければならないか？)
...
- [基礎技術](#基礎技術)
  ...
  - [自由更新](#自由更新)
  - [「それでも何でP2Pなんか必要なの？」](#「それでも何でP2Pなんか必要なの？」)
  - [古い説明。分散合意](#古い説明。分散合意)
  - [古い説明。プロセッサ証明](#古い説明。プロセッサ証明)
  - [古い説明。プロセッサ証明において近傍の近傍数を証明的に確認する方法](#古い説明。プロセッサ証明において近傍の近傍数を証明的に確認する方法)
  - [古い説明。分散合意を通じた全ノードにおけるDBの統一方法](#古い説明。分散合意を通じた全ノードにおけるDBの統一方法)
  - [古い説明。定期的なＤＢの統一処理下における任意のノードが任意のタイミングでＤＢの更新を起こす方法](#古い説明。定期的なＤＢの統一処理下における任意のノードが任意のタイミングでＤＢの更新を起こす方法)
...
- [その他](#その他)
  ...
  - [デフレ](#デフレ)
  - [知識生産経済](#知識生産経済)
  - [情報のローカル化現象](#情報のローカル化現象)

# これは何？  
個人によるプロジェクトです。私はソフトウェアのマネタイズ問題等昔からネットに様々な問題を感じていて、そういった問題を解決するために何があればいいか考えがまとまったので、それを実現するためのソフトウェアプロジェクトを始めました。

このプロジェクトは以下に述べる構想によって、オンラインゲームのプレイヤーや動画制作者、配信者、プログラマー、その他ソフトウェア制作者やオンラインサービスの提供者等が収益化できる状況を目指します。つまりオープンソースや無料サービスやあフリーソフト等について、**有料化する事無く自由な相互利用を保ちながら幅広くネット上での活躍が収益化される状況**を目指します。さらに、様々な創作者が自由に参加できる1つの連合型MMOを構築できます。
  
**Tenyuプロジェクトは、参加者から集金する話ではなく、参加者に仮想通貨を与える話です**。  
仮想通貨はいくらでも作り出せるデータで大勢の参加者に与える事が可能です。いくらでも作り出せるからといって必ずしも価値を持たないわけではありません。例えばMMOのゲーム内通貨もいくらでも作り出せるデータですがRMT等で取引されるなど価値を持ちました。参加者数が多く、めちゃくちゃな運用がされないという信頼性があれば、仮想通貨は価値を持ちます。  
仮想通貨の信頼性は、P2Pであるということで基本的に誰もが全体のデータを持っていて透明性があり、データの遷移が追跡可能であることが大きく寄与します。適切な運用が行われているか誰もが検証できます。  
さらに、参加者全員の投票を通じた意思決定機能があり、運営者を選出したり各運営者の影響力を参加者が決定できます。その点でも信頼性が向上します。投票プロセスや結果は作者である私や運営者ですら改ざんできません。その投票技術は世界的な電子投票を実現しうる重要なものです。  
  
私の考えでは、私がTenyuプロジェクトを通して提案している事は新たな資本強化サイクルです。つまり、政府資本や民間資本等と同次元の新たな概念です。  
このプロジェクトは貢献度に応じて報酬を与える相互評価フローネットワークという仕組みを含みます。もしこの構想が実現するなら、参加して評価を得る事で報酬が得られます。評価は参加者の間で相互に行われます。抽象ノードという仕組みがあり、仕組みとしてはあらゆる貢献を扱えますが、実際にはソフトウェアを主に想定します。
貢献は相互評価フローネットワークに記述され、それに応じて仮想通貨が分配されます。  

もしあなたが情報技術の専門家なら、まずこのプロジェクトが偽物でないことを確かめるためにプロセッサ証明と分散合意について確認してください。それはブロックチェーンを代替するアイデアです。  
  
賃金で働く労働者の立場は基本的に弱いものです。しかし本構想がもし実現するなら、社会的認知や評価、生産活動などを個人レベルで行うので、より独立的な立場を獲得できる可能性があります。普通の労働ができない人がネットには大勢居ますが、そのような人達が稼げる状況を作ろうと思っています。  

**従来の経済は稼ごうと思えば誰かから金を回収する必要がありました**。しかし**Tenyuの構想はネット上の暇な人達が集まって何もないところに価値ある仮想通貨を作り出すというものなので、誰からも回収する必要がありません**。たまにネットで活動している人でファンにカンパを求めている人が居ますが、ファンもほとんどの人が十分にお金を持っていない事に気付く必要があります。

Tenyuは独自のP2P技術によってあらゆるオンライン上の収益化に利用できるようなセキュリティと性能と機能性を持つP2Pのシステムを実現します。私はそれをP2P承認情報基盤と呼ぶ場合があります。
...
# License
この文書及びTenyu基盤ソフトウェアのソースコード及びバイナリのライセンス
TenyuLicense  
...
# 基盤ソフトウェアのソースコードの公開
動作の透明性のため基盤ソフトウェアはソースコードが公開されます。基盤ソフトウェアはTenyu Platform Licenseで公開されます。  
...
最大のリスクはIPアドレスとTenyu上のユーザーIDの対応関係が公開されることです。もともとTenyuに関係無くネット上での行動によってあなたのIPアドレスが知られてしまう可能性がありますが、IPアドレスが分かるとポートスキャンが可能で、あなたが実行しているP2Pソフトウェアが分かります。Tenyu基盤ソフトウェアもP2Pソフトウェアです。そしてTenyu基盤ソフトウェアの機能を通じてあなたのTenyuにおけるユーザーIDが特定可能です。あなたのネット上での活動がIPアドレスを通じて紐付けられてしまうリスクが少し上がります。  
Tenyuの仕様を考えるにあたりユーザーのIPアドレスが知られてしまう問題について解消する方法が見つからなかったので、むしろ利便性を選択し任意のユーザーの現在のIPアドレスが分かる仕様になっています。そもそも全IPアドレスないし膨大なランダムなIPアドレスにポートスキャンする事が可能である事を考えると、その問題を解消する事は不可能でした。それならむしろ正規の機能として搭載して利便性を選択すべきだと判断しました。  
...
他にP2PソフトウェアのためにPCをつけっぱなしにする傾向が強まると雷によってPCが被害を受けやすい。特にLanケーブルは雷対策がされていない事が多い。
...
本構想や基礎技術の発明者が私であることは、2018年1月のTSA、2018年5月の特許の電子出願、2018年7月頃の上記bitbucketの投稿日時によって証明できます。特許の電子出願はマイナンバーカードと対応付けられ、出願者と特許庁のコンピューターに双方の電子署名つきでいつどんな出願が行われたか記録されます。私はソフトウェア特許に疑問を持っていますが、発明者であることを証明するために出願しました。  
...
作者権と著作者人格権の違いは、作者権はあらゆる創作的成果を対象とし著作物の定義に影響されない事です。作者権は発明やアイデアも対象とします。
さらに、作者権によって保護される範囲は著作権よりもずっと狭く単純です。
  
貢献者は共同作業をしたメンバーだけを指すのではありません。多くの場合創作物は完全な独創ではなく他者の創作物を参考にした部分があり、参考にした他者の創作物を明示する事で自分が努力した部分を示す必要があります。  
...
作者権は著作権や特許権などと異なり他者が自分の創作物を利用することを排除する意味合いが無く、**創作の自由と競合しません**。そして、**Tenyuプロジェクトは作者の経済的利益と創作の自由を両立する構想があります**。創作物の自由な利用を促しつつ作者が妥当な報酬を獲得できるという構想です。**もしその構想が成立するなら、それによって作者権程度の主張で十分に作者の経済的利益は達成されるから著作権や特許権のような他者の利用を排除する権利を主張する必要が無い**という考えが背景にあります。  
...
それは**社会を騙したか、不当な評価を得たかで決まる**と思います。創作物を公開する場合不当な評価を獲得しないよう参考にした創作物を明示すべきです。ただし標準化した知識やソフトウェアを除く。
作者権はいわゆる悪質なパクリを否定します。
私が考えるあるべき創作物の相互利用ルールはTenyuLicenseに定義されます。
...
# 人権
私は作者権とプロジェクト権を人権に加えるべきと主張します。ネット時代になり個人のソフトウェア制作や発明が可能になり、ソフトウェアの経済を作る必要があり、それを支えるための基本的権利を定義する必要があります。前述した通り作者権は特許や著作権より単純かつ創発的です。
...
そして**人権が人々の意識へ浸透し、民主主義を通じて人権が力を持ち**、社会全体を長期的に規定します。
...
私が世界的な事を主張するのは、私がソフトウェアの経済を構想していて、経済は基本的権利や道徳や信用が無ければ存在せず、ネットを通じてソフトウェアが世界的に拡散するからです。

人権やその他法規は立法を長期的に規定するので、ひいては社会全体を長期的に規定します。作者権やプロジェクト権を人権に加えた場合、**社会が創作活動を保護するよう長期的に規定されます**。  
インターネットがある時代において作者権とプロジェクト権は世界的に成立すべきです。  
...
以上の理由から民間資本や資本主義といった概念では創作活動をうまく扱えない可能性が高い。
...
「誰でもアイデアを思い付いた時に創作活動をする」そのような事を可能にするプラットフォームを作る事で、マイペースで取り組めます。もし企業だと雇用してみたけど創作的成果が出なかったというリスクがあります。創作的成果の生産は予測不可能だからです。
さらにソフトウェアは他のソフトウェアを参考にしたり部品として利用する事が可能です。相互利用しやすいライセンスを定義してそれでソフトウェアを公開する事で徐々に個人でも高度な作品を作れるようになるはずです。

これらの性質を考慮すると、創作活動のためのプラットフォームは基本的に個人の創作活動を想定すべきであり、総合するとそれはこれまで弱者であることが多かった個人が主役となる時代を主張するものです。とはいえソフトウェア制作は多様であり、私が主張している事が実現され大抵のソフトウェアが個人によって作られるようになったとしても、なお企業が作るべきソフトウェアは存在すると思います。大規模なものやハードウェアに近いものなど。
...
ちなみに私の経験上、ある種の自由度が高いMMOでアイテムやゲーム内マネーを手に入れる事は内発的動機づけをむしろ強化する事があります。理由は分かりません。恐らくそこに「命令」が無く、「自由」であると感じているからです。
...
創作活動も他の活動と同様に多少の金銭報酬が無ければ継続不可能なので、それをどのようなシステムで与えるべきか考える必要があります。
**創作活動における金銭報酬はどの程度創発的現象に貢献したかに応じて与えられるべき**です。
それが創作活動をする者として唯一の健全な外発的動機だと私は考えています。
商業的な売上ではなく、二次創作をどの程度生み出したか、後の作品にどの程度影響を与えたかなどで報酬が決定すべきという事です。
即ちあるべき報酬額は売上と連動せず派生作品の増加にこそ連動するのであり、売上を得ている者が報酬を支払うというアイデアは通用せず、報酬を支払う者は人ではなく**全体を代表する資金が枯渇しない特別な主体**である必要があります。
Tenyuの報酬システムはそれを実現します。

# ソフトウェアの性質
ソフトウェアは複製が容易である事から、生産性のみを考えれば世界的にソースコードを公開し相互利用した方が良い。
しかしそれで成立するビジネスモデルが無かった。
オンラインサービスも無料提供した方が良いものが多いが、やはりビジネスモデルが課題になる。
Tenyuの抽象ノード概念はまさにそのようなビジネスモデルを提供します。

従来の経済は人から人へ価値が提供され、提供を受けた側がお金を支払っていました。
これはものや時間を収益化する事に適していました。
しかしソフトウェアを収益化する場合、その複製が容易であるという性質からソフトウェアを世界の共用財産として他者が自由に利用しやすいライセンスで公開する事が世界の生産性を最大化するために望ましく、その世界の共用財産として供しているという点から、**即ち個々の主体が支払いをするのではなく全体を代表する特別な主体が支払いをするのが妥当**です。Tenyuでそれは共同主体と呼ばれます。
さらに、ソフトウェアは創発的現象を非常に起こしやすく、創発的現象への貢献に応じて収益化されるシステムが望ましい。Tenyuでそれは相互評価フローネットワークと呼ばれます。
さらに、ソフトウェアは世界的に複製される事から、その全体を代表する主体は世界的なP2Pシステムで実現される事が望ましい。Tenyu基盤ソフトウェアはそれを実現します。

# 方法について簡易な説明
作者に支払われる報酬はどこから出てくるか？**高性能かつ堅牢な仮想通貨を実現する独自技術を発明したので、オンラインゲームによってその仮想通貨に価値を持たせてアルゴリズムに従って作者に分配します**。  
...
オンラインゲームをどう作るか？**素材を共有する仕組みがあり、素材やソースコードを公開する事が利益につながる仕組みがあるので、オンラインゲームの制作は徐々に簡単になります**。最初にいくつかサンプルゲームを私が制作します。  
どういうアルゴリズムで仮想通貨を分配するか？**相互評価フローネットワークという構想があり、そのフロー計算によって分配します。そのソースコードは公開されます。相互評価フローネットワークでは人々の間の相互評価に基づいて仮想通貨の分配量が決定します**。  
オンラインゲームはすぐに廃れないか？**多数のオンラインゲームを連携させれます。1つのオンラインゲームが廃れるだけなら構想に支障はありません**。
一般に創作的成果はその背後に複雑な貢献関係があり、フローネットワークを用いなければ正しく貢献関係を記述できません。創作的成果はある程度高度な独自の創作性が入っているとしても100％独自の成果である事はありません。**本構想はフローネットワークを用いる事で複雑な貢献関係を正確に記述できて、そのデータに基づいて仮想通貨を分配します**。  
広く他者に自分の成果を提供し、無料で利用してもらい、参考にした他者の成果を相互評価フローネットワーク上で公正に記述し、自動的に貢献に応じた報酬が各作者に与えられる。
Tenyuプロジェクトはそのための技術と構想があります。  
  
# ソフトウェア特許問題
ソフトウェア特許は多くの人によって問題が指摘されています。  
https://japan.cnet.com/article/35015915/
私が考えるに、その根本的原因は、ソフトウェア特許の抽象度の高さとプログラムが様々な技術を組み合わせて作られる事からソフトウェア特許が多くのプロジェクトに影響を及ぼす事、ソフトウェアの変化の速さに対して特許制度がボトルネックになること、ディープラーニングが当初理解できる人間が世界でも100人以下しかいないという事態になった事実があり審査困難な場合がある事、セキュリティに関する発明はセキュリティの向上を証明する方法が無い事、特許制度が発明の背後にある貢献関係を正確に記述し得ない事、そして後述するハードウェア系分野とのコスト面の性質の違いにあります。  
...
細かなアイデアで出願競争に勝利して特許を押さえてしまおうという行為は発明に取り組んでいるのではなくある種の法的闘争に取り組んでいるのであり、発明者保護ではありません。
...
演算量証明という概念は極めて重要で、Tenyuの基礎技術はそれを応用したものです。
 - 公開鍵暗号、RSA  
...
## 承認情報  
まず、**非**承認情報は例えばニュースや動画など**情報の取得**を本質とします。承認情報はオンラインゲームのアイテムデータ等**情報がどこかに保持される事**を本質とします。オンラインゲームのアイテムデータは従来の方法ではゲームサーバーに保存されます。もしゲームサーバーのDBをもらってもプレイヤーは嬉しくありません。**大勢のプレイヤーが共用しているゲームサーバーにデータが保持される事**が本質です。そのような情報を承認情報と呼びます。  
P2P承認情報基盤は承認情報の保存先がP2Pネットワークであり、透明性においてC/Sを大きく上回り、管理者による恣意的改ざんを受けません。さらに参加者が管理者を任意の他のユーザーに交代させれます。  
承認情報は例えば各ユーザーのアイテム所有情報や仮想通貨残高です。
  
## 承認情報基盤  
承認情報のCRUDが可能で恣意的改ざんを受けないシステム。それが可能になる事で主に多主体の系で情報の統一がもたらされ、承認情報が高度に発達する。  
例えばオンラインゲームのサーバーは各プレイヤーキャラクターのレベルやアイテム所有情報等の承認情報を作成、更新、あるいは削除して、それら情報を多くのプレイヤーに送信し、承認情報のCRUDを実現しています。管理者を除けば不正な改ざんはおおむね困難です。

## 情報の統一  
多主体の系において共通の情報を持つ事、つまり情報を統一する事は基本的に困難で、様々な工夫が必要です。多主体の系とは多数の人間が居る社会、または所有者が異なる多数のコンピューターが参加するP2Pネットワークです。誰かが情報の恣意的改ざんを試みるかもしれず、それに釣られてネットワークの過半数が改ざんされた情報に染まる事を防止する必要があります。その防止が達成されると価値が高い承認情報を保持できるようになります。  
  
## P2P承認情報基盤  
承認情報基盤をP2Pネットワークで実現する事が可能です。P2Pは承認情報基盤としてC/Sと決定的に異なります。C/Sは運営による不正行為やサービス終了を防げません。P2Pは誰もが状態遷移を監視できて不正を監視可能で僅かでも需要がある限りサービスが継続されます。  
  
## 天祐Tenyu
Tenyuは本構想を実装した私が作成しているプログラムの名前です。Javaのアプリケーションで、P2P承認情報基盤です。  
Tenyuは分散合意と演算量証明等に基づいた近傍評価及び同調処理という発明によって、高性能なP2P承認情報基盤を実現します。  
さらに、その技術によって、本構想ではユーザーが全体運営者を選挙的プロセスによって選出する事が可能です。  
Tenyuのあらゆる承認情報は堅牢性が保証され、過半数の参加者が健全である限り保存された承認情報は恣意的な改ざんを受けません。全体運営者や開発者ですら恣意的な改ざんが出来ません。ネットワークの過半数のノードが協調して改ざんしようとすればできますが、その場合その改ざん内容は周知されているのであり、透明性があるという事になります。  
大勢の攻撃者が居れば一部のノードに不正な値を見せる事は可能ですが、過半数の参加者に不正な値を見せる事はできず、過半数の参加者の持つ値がネットワークに広まります。Tenyuの承認情報は完全な透明性があり、状態遷移が追跡可能で、誰でも検証できます。ネットワーク勃興から最新状態までの全状態遷移を再現できます。  
誰でもTenyuで共有された全ての情報が取得可能で、そこにユーザー一覧や各ユーザーのアイテム所有情報や仮想通貨残高やユーザー間の相互評価等重要な情報が全て置かれているので、誰もがそれらの情報と連携したサービスを開発できます。  
TenyuはP2Pネットワークを通じて**改竄困難な投票を実施できて、各運営者の影響力の設定ができたり、アンケートが実施できて、全ユーザーによる意思決定が行えます**。
...
通貨は汎用的な事業形成能力として捉えられます。通貨以外でも設備や周囲からの認知など様々なものが事業形成に貢献します。それらは資本の一部です。  
反復的な自己強化サイクルとは、例えば資金や設備や経験や周囲からの認知を強化していく事を意味します。

私はこのTenyuプロジェクトを通じて主張している資本形態を共同資本と呼びます。
うまくいけばそれは仮想通貨の価値、TenyuLicenseのソフトウェアの数や規模、相互評価フローネットワークの複雑さ、ユーザー数を反復的に強化します。それらはいずれもそれらを強化する理由になるからです。
...
- 特殊な選挙的プロセスによる運営者選出
- 完全な透明性
完全な透明性は長所でも短所でもあり、例えばプログラミングを通じて共同資本から報酬を受け取るには基本的にソースコードを公開する必要がありますが、その制約は場合によって致命的となるので共同資本が扱える領域を限定します。
...
共同資本は世界の共用財産となるソフトウェアや知識の生産に向いた資本です。
ソフトウェアを他者が利用しやすいライセンスで公開し世界の共用財産とする等最も創発的な選択をする事が前提にあり、報酬の支払者は共同主体という全体を代表する特別な主体です。
本P2Pソフトウェアがもし世界的に普及したという前提で考えると、共同主体から報酬が支払われる事は「世界の共用財産への貢献に応じて報酬が支払われている」と解釈できます。
誰にいくら報酬が支払われるかは多数のユーザーによってつくられる相互評価フローネットワークによって決定されます。
このシステムは完全な透明性があり、誰も恣意的に改ざんできません。

後述する仮想経済によって仮想通貨に価値を持たせ、相互評価フローネットワークによってクリエイターに仮想通貨を分配し、制作者はエッジ獲得のためにソフトウェアを制作しTenyuLicenseで公開し、ソフトウェアが共有される事で他の人が新たなソフトウェアを作り易くなり、新たなソフトウェアが作られる事で仮想経済が加速し仮想通貨の価値が向上し、TenyuLicenseに従ってエッジを作成していくことで一貫したルールの元で相互評価フローネットワークが発達し、相互評価フローネットワークの発達と安定が新たな参加者を呼び込み、これらサイクルが反復的に繰り返され強化され続けます。
...
共同資本は世界で１つであるべきです。インターネットで世界的な情報の拡散が容易に発生するからです。ソフトウェアや知識の拡散に障壁を作る事は困難です。ネットがある事でソフトウェアや知識の生産において世界的な相互の貢献が容易に起こるので、その貢献に基づいた報酬システムは世界で1つであるべきです。  

## コード資産の共有
ネット上でのコード資産の共有によってプログラマーの不足は解決されます。本構想はコード資産の共有を実現する構想を持ちます。例えば相互評価フローネットワークのOSS抽象ノードからのエッジがそれを実現します。  
  
## 仮想通貨  
仮想通貨はP2P承認情報基盤で管理された量で、たまに総量が追加される事を除けば原則として保存量であり、多くの人々によって通貨として承認されたものです。  
特にTenyuの仮想通貨はその構想上のオンラインゲームにおいてゲーム内マネーとして使用できます。
...
# Tenyuの基礎技術の概要
Tenyuは**高性能な情報更新技術である準P2P型**と**低性能だが純粋に分散的な純粋P2P型**を使い分けて合理的な性能を達成しています。セキュリティと性能を両立しています。
いずれもP2Pネットワークで共有されている情報を更新できて、一度P2Pネットワークで共有された情報は喪失したり改ざんされません。情報の作成過程も改ざん困難だったり誰のどんなメッセージによるものかを証明可能です。  

しかし準P2P型はシングルポイント（メッセージ受付サーバ）があり、そこに障害が発生している間、情報の更新が停止します。
シングルポイントと言っても実際には多重化されていて耐障害性を強化できます。
準P2P型は例えばゲームに関する情報の更新やユーザー登録や仮想通貨の送金処理などに使われているので、障害が発生するとそのような事が機能しなくなります。

純粋P2P型は純粋に分散的で最高の耐障害性があります。
**そこで、純粋P2P型は全体運営者の選出などこの構想の最も基礎となる機能に用いています**。
**純粋P2P型による投票はP2Pネットワークのノード数に比例して改ざん困難になります**。
メッセージ受付サーバが停止して復旧しない場合、
全体運営者の選出に伴い自動的に選出された全体運営者がメッセージ受付サーバの候補に追加されるのでシステムを再開可能です。
...
**TenyuはP2P上で普遍的と思われる技術を複数持ち、要件に応じて使い分けています**。
**Tenyuの設計には特定の管理者に依存しない、最悪な機能停止や破綻から復旧可能であるという哲学が一貫して存在しています**。

ただしソフトウェアを更新する役割は作者である私に依存しています。
もしソフトウェアを更新する人を動的に決定すると、動的に決定される人物が参加者のPCにソフトウェアをインストールさせる権利を持つ事になり、極めてハイリスクになります。そこでそれは静的に決定する必要があり、私の公開鍵とユーザーIDが設定されています。
厳密には、動的に選出された全体運営者と作者の両方の合意があってアップデートが行われます。
...
ただし基本設計通りの性能を達成するにはソフトウェアの最適化をする必要がありエンジニアリングがまだ必要かもしれません。
実際のp2pネットワークにおける性能実験はできておらず、私の想像力と簡易なテストケースによってこのような性能があると考えているだけです。
...
## 相互評価フローネットワークを通じた仮想通貨分配  
相互評価フローネットワークでフロー計算ができますが、フローに応じて仮想通貨が分配されます。分配元は共同主体であり、共同主体の仮想通貨残高の一定割合が定期的に分配されます。誰にいくら分配するかが相互評価フローネットワークで決まりますが、相互評価フローネットワークは大勢のユーザーによって作成されます。  

## プレイヤー報酬
プレイヤーはレーティングゲームでの成績に応じてそのゲームにおけるレーティングが計算されます。そのゲームへの仮想通貨分配の一部がプレイヤーにレーティングと試合数に応じて分配されます。  

# なぜP2Pでやるか？
最も抽象的に述べれば**新しい信頼可能な前提を作れるから**です。
所有者が異なるコンピューターが多数存在するのがP2Pネットワークですが、過半数の参加者が悪質な改ざんをソフトウェアに施す事は考えにくいので、少数派の参加者だけがデータを改ざんしても全体に影響しないような設計にすれば全体としてデータが健全に保たれます。
新しい特性を持つ信頼可能な前提、新しい承認情報基盤が実現されます。
長所はコンピューターによって自動的に処理される事と、プログラム及びデータの完全な透明性です。短所は一人一票ではなくコンピューターの性能に応じて票数が変わる事や、プログラムにバグがある可能性です。  
そして世界的なP2Pネットワークで選挙的プロセスによって選出された全体運営者なるものに任せるべき役割が存在します。
...
本構想の仮想通貨がどのようにその条件をクリアするかを示します。
  
  - それを獲得するための活動が全体利益にかなうこと。
    Tenyuの構想では相互利用しやすいライセンスでソフトウェアが公開され世界の共用資産が増加します。それが新たなソフトウェアを実現します。  
  - 誰もが努力すれば獲得できて努力しなければ獲得できないこと。  
    Tenyuにおいて、通貨の分配経路は相互評価フローネットワークで記述され、一人一人の参加者の判断で作成され多様な基準で作成され複雑化します。この点で多様な貢献が評価されると考えています。基本的に公開された成果に基づいてエッジを作成すべきで、エッジが作成された理由について誰もが検証可能である事が望ましいです。

# ソフトウェア開発の未来
Tenyuは個人規模のソフトウェアプロジェクトが連携する未来を想定しています。ソフトウェア開発の敷居の低さやコミュニケーションコストの高さや個人制作が可能である性質を最大限活用できるからです。もし一人しか開発者が居ないプロジェクトで開発者がやめても、Tenyu Licenseでソースコードを公開していたなら他の人がそのソースコードを継承して開発を継続できます。そのような場合でもオリジナルの開発者は相互評価フローネットワークを通じて適切な評価と報酬が得られます。  
ソフトウェアプロジェクトは高コストな上に失敗率が高いと言われています。コミュニケーションコストが原因にあると思います。   
ソフトウェア開発の未来は、個人規模のソフトウェアプロジェクトの連携、ネット全体での発明やソフトウェア資産の共有、作者の相互評価による妥当な報酬などが達成され、低コストかつ高成功率かつ多様なソフトウェアプロジェクトが実現されると考えています。  
...
# 相互評価フローネットワーク  
ソフトウェア的な貢献関係を記述したネットワークで、このネットワーク上で評価される事で仮想通貨分配を受けられます。  
従来ソフトウェアはマネタイズの方法が確立していませんでした。特許や著作権によるマネタイズは他者によるソフトウェアの自由な利用を阻害することを伴いました。相互評価フローネットワークはソフトウェアのほぼ自由な利用を推進しつつソフトウェアのほぼ汎用的なマネタイズを実現します。  
...
## 有向グラフ  
有向グラフでGoogle検索すると丸と矢印の図が出てくると思います。丸はノード、矢印はエッジです。エッジに重みを設定できます。重みは数値で、エッジの強さのようなものです。フローは、ノードから発生してエッジの上を流れていく量で、より重みが強いエッジほどより多くのフローが流れます。だから、よりたくさんのより強いエッジを受けているノードはより大きなフローを獲得します。  

## エッジ作成操作  
ユーザーはTenyuのGUIからエッジを作成できます。作成されたエッジの情報はP2Pネットワークで統一値として共有され、堅牢に維持されます。エッジ作成画面で、もしあなたが全体運営者に選出されていたら、共同主体からのエッジも作成できます。もしあなたがWEBページにおいてURL証明を終えていたら、WEBページからのエッジも作成できます。もしあなたがゲームの運営者として登録されていたら、ゲームからのエッジも作成できます。  
  
相互評価フローネットワークはある種の有向グラフで、A→BはAがBから貢献を受けた事を意味します。その重みはどれくらい大きな貢献を受けたかを意味します。  
大雑把に説明すると、AさんはBさんがこれくらい好き、AさんはWEBページ１がこれくらい好き、という感じでエッジを作っていきます。  
重みのつけ方は、10000を100％として、1000が10％、100が1％になります。自分に流れ込んだフローの何％がその人に流れ込むか、という事です。流れ込むフローの多さが仮想通貨分配量を決定します。自分が尊重すべき人やものにエッジを作成していきますが、少し尊重できるもの、大いに尊重できるものなどあると思いますが、その差別化をエッジの重みの差として表現できます。  
  
例えばこんな重みをつけたとします。  
![Alt text](readmeImg/edges.png)  
全エッジの重みの合計と各エッジのそれに占める割合   
![Alt text](readmeImg/edges2.png)  
%だけが意味を持つ。  
![Alt text](readmeImg/edges3.png)  
1200のフローが入ったとして、  
![Alt text](readmeImg/edges4.png)  
各エッジの割合に応じてフローが分割されます。  
![Alt text](readmeImg/edges5.png)  
全員がエッジ作成活動をすると、全体として複雑なネットワークができます。WEBページはその制作者や素材提供者にエッジを作成し、ゲームは衛星作品や運営者や創作者にエッジを作成し、創作者は影響を受けた講座サイトや他作品のWEBページにエッジを作成します。総じて言えば、オンライン上の貢献関係についてエッジを作成していくわけです。緑の旗は後述する共同主体を意味します。共同主体からフロー計算が行われ、それに応じて仮想通貨が分配されます。黄色の旗はゲームです。  
![Alt text](readmeImg/flownetwork.png)  

## エッジ作成のルール
エッジは公開または公開予告された成果に対して作成する必要があります。非公開の成果に作成する事はBANの理由になります。  
公開予告とは、TSAサービスによるTSRファイルやハッシュ値を予め宣言しておいて、後からTSRファイルやハッシュ値に一致するファイルを公開する事です。あまりに公開が遅れた場合、やはりBANの理由になります。全体運営者が個別の事例を判断してBANを決定します。  
このルールはダミーアカウントに自作自演でエッジを作成する行為による仮想通貨分配を不正に増やす行為を防止するためです。  
...
次数３です。このようにしてフローが流れ、各ノードの到達フローが計算されます。ただし、次数に上限があり、おおよそ６～７です。始点からあまりに遠いところまで流れないということです。それは単に計算量が大きすぎるからです。次数が１増えるたびに、計算量が１００倍程度に急増します。  
...
そこで、各ノードにおいてフローを次数別に集計したうえで、次数に応じて補正をかけます。  
次数毎に係数があり、到達フローにかけられます。  
係数は全体運営者が設定します。  
この仕様は相互評価フローネットワークによる仮想通貨分配において、中心付近でなくとも十分に仮想通貨分配を受けれるようにするための仕様です。ネットワークの発達に応じて徐々に外側で大きな分配が受けれるように調節する事を想定しています。  
  
## ダミーアカウント問題と格差是正  
ユーザーによって到達フローに大きな差が生じる可能性があり、それは獲得できる仮想通貨量に影響します。  
しかし到達フローの差を縮小するような計算方法、例えば到達フローをシグモイド関数に入れた返値から分配量を決定すると、ユーザーは複数のユーザーアカウントを作成して貢献度を分散させた方がその縮小処理の影響を防げるので、そのような格差是正の仕様はダミーアカウントへの誘因になり、採用できません。  
到達フローに上限値を作る事や差を縮小させる計算式を導入する事は基本的に採用できません。  
つまり実社会の累進課税のようなアイデアはダミーアカウント問題を抱えるネットの世界において通用しません。  
  
## ノードとエッジの種類  
ノードやエッジは種類があります。ノードの種類毎に管理者を決定する堅牢な手順があります。管理者はそのノードからのエッジを作成する権限を持ちます。ノードの管理者でなければエッジを作成できません。エッジの種類はおおよそどのような理由でエッジを作成すべきかを示唆します。  
これらノードは他のノードに向けてエッジを作成できます。  
**ノードの種類**  
  
- 共同主体  
共同主体は全体を代表するノードで、相互評価フローネットワーク全体で1つしか存在しません。全体運営者が管理者になります。つまり分散合意による選挙的プロセスによって管理者が決定されます。共同主体からのエッジは全体に貢献したのがどのノードかを意味します。  
- 抽象  
抽象ノードは共同主体からのエッジを受ける事ができる唯一の種類です。抽象ノードの管理者は全体運営者が指名したユーザーです。一部の抽象ノードは予め定められた基準によって自動的にエッジが作成されます。共同主体からエッジが作成されると書かれていた時、抽象ノードを経由して作成される事を意味している場合があります。抽象ノードはさらにいくつか種類があります。抽象ノードの種類参照。  
- レーティングゲーム  
...
**抽象ノードの種類**  
  - ソフトウェア  
  共同主体のソフトウェア、つまりTenyuを意味します。ここからTenyuのソフトウェア開発に貢献したノードにエッジが作成されます。
  - 運営  
  本構想全体の運営に貢献した人にここからエッジが作成されます。主に全体運営者または全体運営者が特別な権限を与えたノードに作成されます。共同主体から運営抽象ノードへのエッジの重みは予め固定値が与えられていて変化しません。  
  - 環境  
  例えばJVMやWEBブラウザなど広く利用されているソフトウェアに対して共同主体が全ノードを代表してエッジを作成するときにここから作成されます。  
  - アバター  
  全ユーザーにおけるアバターの使用状況を調査してここからアバターのWEBページにエッジが作成されます。
  - 発明  
  基礎技術や仮想経済構想の発明者にここからエッジが作成されます。
  - 立ち上げ協力者  
  本構想の立ち上げに協力した人にここからエッジが作成されます。
  - オープンソース
  有望なオープンソースプロジェクトにエッジを作成します。

**エッジの種類**  
  
エッジはノードが他のノードから貢献を受けた場合に作成します。種類は作成時に選択します。  
エッジの種類によって仮想通貨分配で使用されるかが異なります。例えば運営貢献や参加貢献や部品貢献や制作貢献等は仮想通貨分配に影響しますが、友人エッジは影響しません。
...
## １シナリオ
例えば動画のWEBページがあり、その動画の制作者がTenyuにユーザー登録し、後述するURL証明を行いそのURLの管理者であることを証明し、相互評価フローネットワーク上でそのWEBページからのエッジが作成可能になり、制作者本人へと制作貢献エッジを作成します。他の貢献者へも部品貢献エッジ等を作成します。さらに、制作者は参考にした動画制作講座のWEBページに自分のユーザーから学習エッジを作成します。動画制作講座の制作者は同様に動画制作講座のWEBページでURL証明を行い、自身に制作貢献エッジを作成します。その動画の視聴者がその動画のWEBページに向けてファンエッジを作成します。結果的に、動画視聴者→動画WEBページ→動画制作者→動画制作講座WEBページ→動画制作講座の制作者へとエッジが繋がります。この一連のエッジは、何が何から貢献を受けたかという事を意味しています。後述するフローに応じた仮想通貨分配を通じてエッジを受けている人たちは報酬が得られます。**エッジは一貫して貢献を受けた事に応じて作成されます**。  

## 主旨
ところで、貢献に応じてエッジが作成されるなら、そしてエッジに応じて仮想通貨が得られるなら、自身が持つソフトウェアをオープンソースにするなどして多大な貢献をすれば多大な仮想通貨が得られる事を意味します。**有用なソフトウェアの公開と自由に利用される事が報酬の増加につながる事**が相互評価フローネットワークの主旨です。  
構想が実現すれば、相互評価フローネットワークによってオープンソースに報酬を出せる可能性があります。ハードウェアの設計もオープンソースの場合があり支援できる可能性があります。  
その他、新しいアイデアも文章で説明すればソフトウェアであり、そのような文章から影響を受けたプロジェクトからエッジが作成される可能性があり、本構想によって促進されます。  

## 共同主体の重要性
相互評価フローネットワークのノードの中に１つだけ共同主体と呼ばれる仮想的なノードがあり、画像では緑色の旗で表現されます。定期的に共同主体の仮想通貨残高の一定割合が相互評価フローネットワークを通じて分配されます。共同主体からのエッジを誰が作るかは重要な問題です。共同主体からどれだけフローが届いたかで各ユーザーへの仮想通貨分配量が決定するからです。分散合意によって選挙的プロセスを実施できますが、それによって選出された全体運営者が共同主体からのエッジを作成します。全体運営者はソフトウェアの次のバージョンを指定する権限もあるので、ひいてはソフトウェアによる自動的なエッジ作成についても決定権を持っています。全体運営者は複数選出し全体運営者ごとに影響力を設定する事が可能で、全体運営者の中でさらに投票が行われ、その可決が無ければできない決定もあります。  
![Alt text](readmeImg/PlatformNode.png)  
  
## URL証明  
WEBページはコメント欄など誰でも書き込める部分と管理者しか書き込めない部分があります。URL証明において、WEBページの管理者は管理者しか書き込めない部分にURL証明コードを挿入します。  
...
ところで、URL証明はWEBページにアクセスしてHTMLを取得した人においてURLと公開鍵の対応関係が証明されるということですが、P2Pネットワークの全ノードがアクセスすると過剰な負荷をかけます。しかし、過半数のノードがURLと管理者公開鍵の対応関係について納得しなければ、統一値になりません。P2P承認情報基盤はその基礎技術によって選挙的プロセスを実施できて全体運営者を選出できますが、全体運営者はWWWアクセス役など特別な役割を指定したユーザーに与える事ができます。URL証明におけるWEBページへのアクセスはWWWアクセス役に指定されたユーザーのみが行います。WWWアクセス役に選ばれたユーザーはFQDNを持つ必要があります。FQDNはユーザープロフィールの一部で、統一値として共有されます。FQDNは"www.xxx.org"などの文字列で、IPアドレスに変換できる情報で、IPアドレスは日々変化しますがFQDNは不変で、FQDNとIPアドレスの対応関係はDNSという標準的サービスによって知ることが出来るので、ユーザーがFQDNを持ちFQDNが統一値になるということは誰でもそのユーザーにアクセスできることを意味します。そして、ここでは、誰でもWWWアクセス役に自分のWEBページのURLを送る事ができて、URL証明コードを確認するよう要求できることを意味します。WWWアクセス役は依頼されたURLにアクセスし、URL証明コードを確認したら、P2Pネットワークに自身の電子署名をつけて報告します。各ノードはWWWアクセス役の電子署名があることをもってその報告を受け入れ、WEBページと管理者公開鍵の対応関係が統一値になります。  
...
TenyuLicenseは著作物の利用者に相互評価フローネットワーク上で公正なエッジ作成を行う事を要求します。  
ライセンスに定義されている公開鍵の宣言はURL証明コードで十分です。  
用途制限を記述する事でTenyuLicenseの著作物の作者は随時その著作物の用途を制限できます。例えば自分が作成したキャラクターのモデルに愛着を持っている場合アダルト用途に使われるのが嫌かもしれません。どのような用途を拒否したいか場合によって異なります。そこで、作者は用途制限を記述できて、随時変更できます。
...
# MMOとは何か  
私が初めてMMOをやったのは14歳の時で、PHIというMMOでした。世界に降り立った直後、何をするゲームなのか分からなかったので、周囲に居た人に尋ねました。「このゲーム何をしたらクリアなの？」相手はこう答えました。「MMOだから、クリアとかないよ」私はそれまでSFCやPSのゲームしかやったことが無かったので、当初その言葉の意味が分かりませんでした。  
...
MMOの重要な特徴は巨大なRMT市場を実現したことです。**MMOは仮想経済です**。
MMOでは、フィールド狩りや戦争やインスタンスダンジョン等様々なコンテンツがありますが、それらはどのように設計してもいいわけではありません。大勢のプレイヤーが報酬のために貪欲に活動するから不正行為が発生したり、報酬が強いコンテンツにプレイヤーが集中し報酬が弱いコンテンツが廃れたりします。**膨大なプレイヤーが貪欲に参加しつつ多様なコンテンツが常に活性化される理想的な動態を作り出す事**が仮想経済の設計の目標です。

本構想はMMOが進歩したもので、仮想経済という問題領域への独自の回答を持ちます。さらに、**仮想経済が自由参加する多数のクリエイターの元で実現されます**。

# 仮想経済
仮想経済とはRMT市場を創出しうる仮想的な経済です。
本構想では相互評価フローネットワークで分配される仮想通貨に価値を持たせる役割があります。
相互評価フローネットワークは仮想経済ではありません。仮想経済は本構想におけるゲーム部分を指します。
本構想の仮想経済はMMOを根本的に設計しなおしたもので、連合型MMOです。
従来のMMOは持続可能なモデルではありませんでした。MMOは多くの場合すぐに廃れてしまいます。廃れてしまえば蓄積されたゲーム内資産は価値を失います。
連合型MMOは1つのゲームが廃れても全体に支障が無く、ゲーム内資産やコミュニティの価値を持続させれます。

以下で述べる事は本構想の仮想経済がどうデザインされるかです。
Tenyuが扱うゲームはレーティングゲームと常駐空間ゲームに分けられます。
...
## 全体運営者  
全体運営者は分散合意で全体運営者として選出されたユーザーです。
...
私はTenyuの仮想経済を連合型MMOと呼ぶ場合がありますが、全体運営者とはその連合全体の運営者という事になります。
全体運営者はゲーム部分だけでなく相互評価フローネットワークを通じたソフトウェア制作の汎用的な収益化も実現する必要があります。
  
## プレイヤー  
ゲームに参加するユーザー。ゲーム毎にユーザー登録する必要は無く、全ゲーム共通のユーザーアカウントが使えます。
常駐空間ゲームで材料を獲得したり、各ゲームで装備を購入したり、レーティングゲームの試合を通じてレーティングが算出され仮想通貨を獲得したりします。  
レーティングゲームでは試合の結果に応じてレーティングが計算され、レーティングと試合数に応じて定期的に仮想通貨が分配されます。レーティングゲームとして多数のゲームが登録されますが、それらゲームは相互評価フローネットワーク上にノードを持ち、ゲームのノードに到達したフローでそのゲームにおける報酬が変動します。つまりアイテムが良く売れるゲームほど分配額が増えます。あるゲームが獲得した報酬全体の一部がプレイヤーへの報酬になりますが、その比率は全体運営者が決定します。  
各ゲームの運営者は報酬が支払われる最低レーティングと最低試合数を設定できます。不正プレイヤーはBANされるまでに最低レーティングと最低試合数を超えなければ報酬が得られません。  

## ゲームクリエイター  
ゲームクリエイターは常駐空間ゲームやレーティングゲームを作成します。基本的想定として、ゲームデザイン、プログラミング、運営を一人で行います。どのような行為を不正行為と考えるかもゲームデザインの一部だからです。  
Tenyu基盤ソフトウェアはゲームクライアントの登録機能を備えるのでそこから登録できます。
さらに、ゲームの公式URLを作り、相互評価フローネットワーク上でゲームから作者へのエッジを作成します。クライアントソフトウェアのソースコードをTenyuLicenseで公開します。クライアントソフトウェアはソースコードを公開する必要があります。実行可能ファイルを含むので、ウイルスやスパイウェアの可能性を排除する必要があるからです。
![Alt text](readmeImg/ServiceNode.png)  
![Alt text](readmeImg/ServiceNode2.png)  
  
ゲームクリエイターは自分のゲームのアイテム品目をTenyuに登録します。プレイヤーは仮想通貨でアイテムを購入します。ゲームクライアントはプレイヤーが所有するアイテム情報をTenyu基盤ソフトウェアから実行時に取得できます。支払われた仮想通貨は共同主体に回収され、同時に回収額に応じて共同主体からそのゲームへのエッジが強化されます。共同主体→ゲーム系抽象ノード→ゲーム→ゲームクリエイターという経路ができて、仮想通貨分配がゲームクリエイターまで届きます。  
  
ゲームクリエイターは利用した素材等について公正にエッジを作成する必要があります。Tenyuに登録された素材はゲームに同梱しなくても実行時に基盤ソフトウェアからロードできます。ゲームエンジンも基盤ソフトウェアに同梱されるので、ゲームのファイルサイズをかなり小さくできます。

ゲームはユーザーのアカウント作成等の処理を扱う必要がありません。Tenyuがユーザー機能を提供します。  
ユーザーはTenyuにアバター情報を登録していて、ゲームはTenyuから実行時に動的にアバターのファイルを取得できます。アバターが取得できなかった場合のデフォルトアバターを用意する必要があります。  
  
従来存在した多くのMMOは仮想経済の設計に失敗していて問題を抱えましたが、Tenyuはそれを提供するので仮想経済の設計に悩む必要がありません。具体的にはレーティングと試合数に応じた仮想通貨分配や材料アイテムを消費して装備アイテムを購入するシステムがあります。
...
## 素材クリエイター
作成した素材をTenyuに登録し、素材から作者である自分へエッジを作成します。他の人が例えばゲーム制作においてその素材を利用した場合、そのゲームからその素材にエッジが作成されるので、共同主体→ゲーム抽象ノード→ゲーム→素材→素材作者作者という経路が成立し、素材の作者は仮想通貨分配を受けます。  
...
- 常駐空間ゲームは様々な条件でプレイヤーに材料アイテムを与える。例えばボスモンスターの討伐など。どんな条件でどんな材料アイテムを与えるか、そもそもどんな品目があるかは各常駐空間ゲームが自由に決定し基盤ソフトウェアを通じてP2Pネットワークに登録できる。基盤ソフトウェアのAPIを通じて常駐空間ゲームは任意のユーザーに材料アイテムを与える。
- 各ゲームはそのゲームで購入できる装備アイテム品目を基盤ソフトウェアを通じてP2Pネットワークに登録するが、装備の購入に伴い消費する材料を設定できる。他のゲームが産出する材料でも設定可能。
...
この代替案はクエストと比べると「NPCによるストーリー」が無くなっています。ストーリーはオンラインゲームの複雑な要素や多数のプレイヤーの中で自然発生すべきと考えています。
...
仮想通貨の獲得手段は相互評価フローネットワークのフロー計算を通じた仮想通貨分配とプレイヤー報酬と他ユーザーからの送金等に限定されます。  

## 装備
プレイヤーは装備を購入しゲームで有利になります。購入可能な装備の品目は各ゲームの運営者が登録します。装備購入の際に仮想通貨や材料が必要です。
ゲームAの装備をゲームBで使える場合があります。

## 材料
材料は常駐空間ゲームをプレイする事で獲得できます。レーティングゲームは材料を生産しません。材料は装備購入の際に消費されます。  
常駐空間ゲームBが生産する材料をゲームAの装備の購入条件にすれば、ゲームAのプレイヤーはゲームBに流れます。そのような事をゲームAとBが互いにやる事が考えられます。そのように各ゲームは連携できます。
  
## ゲーム
常駐空間ゲームとレーティングゲームがあります。ゲームは仮想通貨に価値を持たせる役割があり、本構想のエンジンです。
...
## 常駐空間ゲーム
常駐空間とは、MMOのいわゆるフィールドです。常に存在する空間で、プレイヤーが自由なタイミングで訪れて自由に行動します。多くの場合常駐空間では明確な勝利条件は無く様々な活動が可能です。典型的には常駐空間ゲームとは**PvE=環境との戦い**です。そのような環境において他のプレイヤーの存在は環境の一部です。プレイヤー動態に依存してその時々の状況が存在し、その中で何らかの条件を達成すると材料アイテムがもらえます。  
C/S型のネットワーキングで提供されます。PvEベースです。PKがあっても問題ありません。装備売上に応じて対応する抽象ノードからエッジを獲得します。材料を産出します。    
![Alt text](readmeImg/ServiceNode.png)  
...
ゲームクライアントはTenyuから各プレイヤーの所有装備一覧を取得できます。プレイヤーは試合の中で所有している装備を使用できます。レーティングゲームは装備売上に応じて対応する抽象ノードからエッジを獲得します。
...
## レーティングゲームのチート対策まとめ
 ...
 - 試合終了後、誰がどんな装備を使用したか、その人が本当にその装備を所有しているかを検証する。基盤ソフトウェアはその時点で誰がどの装備を持っているかを知っているので判定できる。問題があれば自動的にチートとして報告する。
 ...
 - 試合のマッチングはランダム性があり対戦相手を自由に決めれないので結託不可能。チート行為をすれば高確率で相手プレイヤーによって報告される。もし過疎な時間帯等でマッチングのランダム性が低かった試合はそれを意味するフラグが立つので運営者が検査しやすい。  
...
- Tenyuに登録されない各ゲーム内のアイテム
  常駐空間ゲームがその空間内のみで使用するアイテム、レーティングゲームが1試合の中でのみ使用するアイテムは自由に作成できます。典型的には消耗品や常駐空間のハウジング要素などが想定されます。  

## マッチングサーバとレーティング
レーティングゲームでは試合が開催されますが、そのマッチングをする必要があります。
全体運営者が指定したノードがマッチングサーバを担います。マッチングサーバはタイムゾーン毎に設定されます。**Tenyuは全てのレーティングゲームに共通のマッチングルールとレーティングシステムを提供します。**  
SMEという計算方法があります。
...
オンラインゲームは海賊行為に耐性があります。サーバーに特定のデータが保持される事、つまり承認情報とその周辺のユーザー動態が価値だからです。  
  
オンラインゲーム主軸モデルは、クリエイターがオンラインゲームを盛り上げるような衛星作品を作り無料で公開し、それをオンラインゲームの収益力で収益化するというアイデアです。例えばオンラインゲームのチュートリアルは衛星作品のオフラインゲームとして作る事ができます。実在のオンラインゲームを題材にした動画や漫画を作る事も考えられます。
オンラインゲームの売上から衛星作品の制作者に報酬を支払います。具体的には、相互評価フローネットワークでオンラインゲームから衛星作品へエッジを作成すれば報酬を与えれます。  

つまりこのアイデアは、海賊行為に対抗するためにソフトウェアを無料で配布し、人々の価値観を海賊行為に耐性があるコンテンツであるオンラインゲームに移行させます。  
  
## 衛星作品
動画、配信、漫画、フリゲ等を想定した概念で、
**対象とするオンラインゲームへ人を誘導し盛り上げる**ような内容を持つ作品です。

## ゲームの条件  
不正行為がはびこったゲームはBANされます。ゲームがBANされると誰もそのゲームをプレイできなくなります。
...
## 前提とするゲームエンジン
今のところゲームエンジンはjme3を前提にしています。
基盤ソフトウェアがビットマップフォントやjme3のjarファイル等を同梱しているので各ゲームは同梱する必要がありません。

jme3 sdkを使用する場合、日本語表示可能フォント(Meiryo等)、ショートカットの設定が必要です。初期設定では使用メモリが小さすぎるので、XmsとXmxを4096mくらいにしてください。このバージョンのNetbeansはデバッグ実行が遅い場合があるので、その場合ブレークポイントを全て削除する必要があります。この情報はsdkがアップデートされるとすぐに古くなる可能性があります。
...
ある程度のところで仕様を凍結すべきです。
アップデートによってプレイヤースキルがリセットされる面があります。仕様が凍結されることでそれを防止できます。ある程度プレイヤーを得て、膨大なプレイ時間を得たゲームは、むしろ仕様凍結によってゲームの寿命を延ばせます。プレイヤー側に蓄積されたプレイヤースキルがそのゲームの競争力になるからです。

仕様凍結後、性能やセキュリティのためのアップデートか、例外的にプレイヤーの新たな工夫によってゲームバランスが崩壊する危機に瀕した場合のみアップデートします。
...
## コンテンツ消費問題
従来の多くのMMOはコンテンツ消費型と呼ばれ、プレイヤー全体のレベルが高くなると低レベルのマップ、装備、スキル等が使用されなくなるという問題がありました。そしてレベルキャップの解放を繰り返しながら死んだコンテンツを増加させました。  
コンテンツ消費型は投入した労力が無駄になる事やクライアントソフトウェアの肥大化、ゲームの仕様が複雑になりすぎてプレイヤーの学習コストが高くなる等の問題があります。
プレイヤーは最新の保守されている意味のあるコンテンツのみを選択して効率よく自分のキャラクターを育成する必要がありますが、死にコンテンツを選択すると無駄な時間を過ごす事になるので、死にコンテンツはプレイヤーにとって罠です。
やがて開発者も全仕様を把握できなくなります。
本構想の仮想経済はコンテンツ消費型にならないモデルになっています。  
  
## エンドコンテンツ問題
上述した通りコンテンツ消費型は継続不可能で、良くエンドコンテンツが必要だと言われます。
本構想でエンドコンテンツはプレイヤースキルの追及または仮想通貨の蓄積または創作者になる事です。  
  
# 基礎技術  
  
## ノード、エッジという言葉  
P2Pネットワークのノードやエッジについて言っている場合と、相互評価フローネットワークのノードやエッジについて言っている場合があるので、混同しないよう注意してください。P2Pノード、P2Pエッジと言っていたら確実にP2Pネットワークの事を言っています。P2Pネットワークはごく標準的な意味のP2Pネットワークですが、相互評価フローネットワークはP2Pネットワークの客観です。P2Pネットワークの全貌はどのノードも知りませんが、相互評価フローネットワークの全貌はどのノードも知っています。  

## 客観
P2Pネットワークで動的に共有された情報を客観と呼んでいます。ソースコードのObjectivityObject系のクラスが相当します。  
  
## ストーリー
最初、全ノードは互いを全く信用していません。  
つまり、全体運営者が居ないし、仮想通貨残高など価値ある社会性情報が一切ありません。
ただし、相互にアドレス情報を交換してP2Pネットワークを形成します。
それは誰かに特権を与えるとか利益を与える事ではないので、信用を必要とする事ではありません。
そうしてP2Pネットワークを作ってみても、まだ誰も互いを信用していないので、自分は仮想通貨をこれくらい持っているぞと誰かが言い出しても
一切信用されず意味がありません。
この段階では全ノードで統一された情報というものは基本的にありません。（基盤ソフトウェアによって与えられる定数等を除けば）
基盤ソフトウェアはプロセッサ証明の日時を定数として持っています。
その日時が来ると全ノードは一斉に近傍にランダム値を含む問題作成情報を送ります。A
各ノードは通常多数の近傍を持っているので、多数の問題作成情報を受け取ります。
受け取った全問題作成情報からハッシュ値を作り、ハッシュ値から一意に定まる動的な問題関数を作成します。B
このとき全問題作成情報に任意の値を加える事ができます。結局ハッシュ値を作れればいいのと、問題作成情報に依存してさえいれば有効であり、他の情報に依存してはならないという制約はないからです。
例えば問題の番号を回答者側で勝手につける事で複数の問題を作成できます。
計算能力に余裕があるノードは多数の問題を解いてその分多くの投票権を獲得できます。
問題関数は引数を取ります。その出力が特定の範囲になるような引数を探索します。
このようにすると、引数探索より引数の検算の方が簡単にできるので、解くのは難しく確かめるのは簡単という事になります。
問題関数はAにおいてランダム値に依存している事から予め作成しておく事ができません。そして予め答えを計算しておく事もできません。
そして問題を解いたら近傍に回答として送信します。このとき回答に全問題作成情報を含めます。
その近傍が送ってきた問題作成情報以外の問題作成情報も全て含めて送信するという事です。C
回答を受け取ったノードは、その全問題作成情報の中に自分が送った問題作成情報が含まれている事、
その全問題作成情報から作成される問題関数において回答に含められた引数が正しい事を確認します。
このとき、AとBから、その回答を送ってきたノードは今その計算をして回答をしたのだと分かります。
そうして初めて全く相互に信用していなかったノードが互いを少し信用します。
つまり、少なくともプロセッサを持っている相手であると信用します。
プロセッサを持っているからなんだというのか？
もしネットで投票をするなら、ダミーアカウントを防ぐ必要があり、プロセッサの存在が信用された事は重要です。
Cの重要性について。Bは1つの問題関数を解くだけで複数の近傍において信用が得られる事を意味します。
...
これによって膨大な近傍を持つ事によってP2Pネットワークにおいて過剰な投票権を獲得するという攻撃を防げます。
ところで、あるノードの元でそのいくつかの近傍がプロセッサを持っている事が信用されたとして、
それはあくまでそのノードの元でのみ有効な信用であり、その信用を他のノードに伝達する術はありません。
そのような事をすれば過剰に信用を生み出すハッキングの可能性が生じます。
ところが、分散合意というアイデアを加える事でネットでの投票を実現するにあたり信用を伝達する必要はありません。
分散合意とは、近傍との局所的多数決を全ノードが一斉に繰り返す事で、1か所に集計した場合と同じ結果になるというP2Pネットワークの現象です。
つまり、局所的多数決をするだけで良いから、信用を伝達する必要が無いという事です。
信用はただ自分の元で存在すればいい。それで局所的多数決が可能です。
そして全体運営者投票など、ネットワーク全体での意思決定が可能になります。

※基礎技術の話ではありませんが、作者が最初のユーザーになっていて、ユーザー登録は最初からできるようになっています。これは作者ノードを最初から信用している事を意味しますが、P2Pネットワーク勃興時の例外であり、ある程度ユーザーが増えたら単に前述した論理によって実行されます。

そうして全体運営者の選出に成功します。選出されたノードは自動的にいくつかの権利が与えられ、P2Pネットワーク上で特別な役割を果たせるようになります。**投票によって選出された人が決定した事だからという理由で各ノードはその決定に従います**。全てを分散合意で決定するのはユーザーの操作が大変ですが、特別な権限を持つノードを選出する事で大抵の役割をそのノードに任せる事ができます。
全く相互に信用が無かったP2Pネットワークが特別な権限を一部ノードに担わせる事に成功しました。
P2Pネットワークを使って何をするかを考えた時、特別な決定を下せる全体運営者の存在は助けになります。全てを純粋に分散的に決定するのは性能や多重送金的問題で難しいからです。**分散的に選出された中央集権的ノード**を受け入れる事はP2P分野の可能性を広げます。

## P2P技術と多重送金とオンラインゲームと相互評価フローネットワークとゲームミドルウェア
良く仮想通貨は多重送金の問題があると言われます。これはメッセージを反映する順序がノード毎にまちまちになる事で発生します。
仮想通貨を100持っているノードが、ノードAとBそれぞれに100送金したとします。残高的に片方の送金しか成功しません。
ところが、その送金メッセージを受け取ったノードが受け取り次第反映処理を行い、さらにメッセージを拡散させていくという動作（私はこれを自由更新と呼んでいます）をすると、ノード毎にどちらのメッセージを先に反映したかが異なる可能性があります。
多数派を優先して情報を統一する事は可能ですが、少数派が信じていた値が消失します。もし仮想通貨の送金に応じて現実のサービスが生じた場合、送金処理が消失する事は重大な問題です。既に提供されたサービスを返品してもらう事はできないからです。物品であったとしても面倒です。
多重送金問題は実はオンラインゲームと重要な関りがあります。もしP2Pネットワークがオンラインゲームのアイテムデータを直接管理するなら、アイテム購入メッセージの反映処理は送金とアイテム獲得をまとめてやる事になりますが、もしアイテム購入処理が消失した場合、送金とアイテム獲得が同時に消失するのであり、どこかに不当な利益を残すという事がありません。つまり購入する価値が純粋にオンラインの処理だけで済むというオンラインゲームの性質が重要なのです。
他にもオンラインゲームのプレイヤーからすると、ゲーム運営による不正なデータの改ざんがほぼ不可能で、あったとしても確実に分かるとか、TenyuLicenseベースのオンラインゲームならもしサービスが終了しても誰かが制作を引き継げるとか、メリットがあります。
オンラインゲームをP2Pベースで作る動きが活発になったら素晴らしいですが、一体オンラインゲームの制作者達にどんな動機があるのでしょうか。ここまで話した事はP2Pソフトウェア側のメリット及びプレイヤー側のメリットです。制作側にメリットはあるのでしょうか。
相互評価フローネットワーク、TenyuLicense、ゲームミドルウェア構想が制作者に利益を与えます。ゲームミドルウェアと言っているのはアカウンティング、レーティング、マッチングサーバ、ゲームクライアントの配布、アセットの配布、ゲーム内通貨、購入システム、アセット共有、演出マネージャーなどの機能を提供する事です。
相互評価フローネットワークによる仮想通貨分配があるので相互評価フローネットワークはP2Pベースにする必要があります。分配計算をC/Sに頼る事は透明性を失わせます。ゲームミドルウェアがなぜP2Pベースでなければならないか。C/Sはその運営者の都合でサービスが終了したり、他のサービスとの連携を拒否する可能性があります。さらにゲームが使用するアセットを共通化しつつ、随時更新できます。
という事で、一見全く関係が無さそうなこれらの概念は関係があります。  

## 自由更新
仮想通貨送金、仮想通貨によるオンラインゲームのアイテム購入、その他いくつかのメッセージは自由更新です。

## 「それでも何でP2Pなんか必要なの？」
説明が難しいです。ただ、やはり世界的な改ざん困難かつダミーアカウントにも対抗できる投票機能は重要な気がします。それだけを実装しても全く流行らないと思いますが。一応様々な投票ができるようになっているものの、投票プラットフォームを目指しているかというとそうでもなく、主眼がどこにあるのか、良さそうに思えるアイデアの連携の末に結局これは何なのか、分からないでいます。  
  
以下、古い説明は、間違いではないものの説明の仕方に苦慮していた時のものです。
...
## 古い説明　プロセッサ証明において近傍の近傍数を証明的に確認する方法  
...
## 古い説明　定期的なＤＢの統一処理下における任意のノードが任意のタイミングでＤＢの更新を起こす方法  
...
ノードは任意のタイミングでオフラインになったりオンラインになったりするので、固定的な連絡網を構築するというアイデアがあまり通用しません。任意のノードから生じるメッセージを拡散させなければならないというのも連絡網の設計を難しくします。
10Gbps回線が普及すれば純粋P2P型でまともな性能を出す事が出来ますが、その場合でも準P2P型なら10倍以上のスループットを達成できます。
...
その検索結果を送信先とする場合、ブロードキャストではなくマルチキャストです。
特定のP2Pネットワークの全ノードへの送信は、そのP2Pソフトウェアによるサーバープロセスを検索してその検索結果全体をあて先としているという事になる。
検索結果の地理的密集度に応じて無線と有線を使い分ける事になるだろう。
アプリケーション非依存の汎用的な検索を実現できる。サーバプロセスから提出された文字列表現された任意個数の特徴をISP側システムが扱い、そのISP側システムが検索サービスを提供する。

もし「インターネットは本来ISPに依存せず純粋に分散的に構築されるべき」という思想を持つなら、その思想に基づく自発的なインターネット網の構築機能を実装したP2Pソフトウェアというものを考えると、それは標準的なプラットフォームソフトウェアであるはずで、その全ノードへの送信はブロードキャストを意味する事になります。
この観点ではPCが無線装置を搭載したり周辺の利用可能な装置を認識して自発的にインターネット網を構築するようなシステム像になる。広域無線送信装置も海底ケーブルもISPの通信を中継するサービスも利用可能な1つの装置として周辺のPCから自発的に認識され利用される。
P2Pネットワークのノード間リレーにおいて周辺の利用可能な装置を認識して効率的な経路をPCが自発的に選択する。
そのシステム像においてもメッセージの重複受信は生じるが、メッセージのフォーマットを規格化しネットワークインターフェースで重複したメッセージの受信を防げば効率的だろう。

P2Pネットワークの全ノード一覧を管理するスーパーノードを用意して、そのノード一覧を送信先一覧とするか？
しかし１メッセージ400バイトで1億ノードを想定すると、1メッセージを全ノードに送信するのに40GBほどの送信になる。これはバカげているし、全ノード数の増加に応じて送信能力が不足するノードが増えていく。

重複送信を防ごうとしてメッセージに送信済みノード一覧のような情報を記録するとメッセージのサイズがでかくなりすぎる。
他に低レイテンシなノードのグループ、グループによる可用性、グループの連絡網、適応的な拡散経路の多重化などを考えた。特にグループによる連絡網というアイデアは重複受信を抑えれるので良さそうだが、耐障害性と効率のトレードオフになる。連絡網というアイデアは、P2Pネットワークをメッセージが拡散していくときの経路が重複しないようにする面がある。重複が発生するとしたら低レイテンシグループ内に限定される等経路の重複に対策できる。しかし、経路をある程度多重化しないとどこかで拡散が途絶するかもしれない。しかし多重化すると効率が落ちる。ここで経路と言っているのはプログラムにおけるスレッドみたいな、リレーの線だ。
適応的な拡散経路というアイデアは、各ノードがAIを使って学習する事で重複受信が徐々に取り除かれる。各ノードは時間帯等を考慮してどのノードにメッセージを中継するかを判断する。同調処理において客観DBの不整合が検出されたら近傍ノードに対して拡散経路を再学習するように要求する。しかしこのアイデアはどの程度の精度が出るか予測困難。

TenyuのP2Pネットワークにとって致命的なのはノードによって客観DBの内容が異なる事であり、メッセージを全ノードが受信するか全ノードが受信しないかどちらかである事が望ましい。

低レイテンシグループは地理的に近い位置に居る事を意味し障害で同時にオフラインになるリスクがある。
さらに、インターネットが地上波や衛星波を駆使した広域送信機能を備えるべきではないか、PCが近所への無線送信能力を備えるべきではないか、そのような様々な通信装置を自動的に適宜組み合わせて必要な送信を達成する汎用的なシステムがあるべきではないか、というような事を考え出すとP2Pソフトウェアがどのように拡散アルゴリズムを実装すべきか分からなくなる。

インターネットのこのあたりの問題がどう解決されるべきかまだ良く分からない。とりあえず現状マシな方法を採用するしかない。

## 存在日時証明TSA
ここで自身が作成したソフトウェアについて自身が制作者であると証明したい者を証明者と呼ぶ。他人のソフトウェアについて自身が制作者であると捏造したい者を攻撃者と呼ぶ。  
P2P承認情報基盤はDBを一斉更新する直前に分散合意によるDB更新メッセージのリストの統一処理があるが、その時、証明者の存在日時証明のメッセージを攻撃者が真似て、同じハッシュ値の存在日時証明のメッセージを分散合意における相互通信情報に含める事で、P2Pネットワークはどちらのメッセージが先に作成されたのかが分からなくなってしまうので、従来と同じ方法では存在日時証明を実現出来ない。  
その問題を回避するための手順を示す。  
証明者は任意のファイルについてそのハッシュ値を秘密鍵で署名しメッセージを作成する。それを署名メッセージと呼ぶ。署名メッセージは電子署名のみを含む。まず署名メッセージが先行してP2Pネットワークにおいて分散合意を通じて統一値となる。  
続けて、署名メッセージが登録された事を確認し、ハッシュ値とユーザーIDを含むメッセージをネットワークに送信し、分散合意を通じて統一値にする。もしその分散合意の中で攻撃者がハッシュ値をコピーして攻撃者のユーザーIDでメッセージを作成して同時に統一値に記録させようとしても、証明者は先行する署名メッセージがあるので自身が先にそのハッシュ値を主張していたことを証明できる。P2Pノードは署名が先行しているハッシュ値メッセージを採用する。  
この手順の特異性は署名メッセージを先行させる事にあり、署名メッセージがハッシュ値を含んでいない事である。攻撃者が署名メッセージを真似ようにも、自身の秘密鍵でまだ公開されていないハッシュ値に署名しなければ意味が無いが、ハッシュ値がまだ公開されていないのでできない。そして証明者は後からハッシュ値を送った時、既にそのハッシュ値に対する署名をネットワークに登録できているので、署名を送った時点でそのハッシュ値を知っていた事、つまり先にそのファイルを持っていた事を証明できる。  
...
しかし「人と競いたい/関わりたい」という欲求はAIによって置き換えられない仕事を作り出します。
Tenyuの構想で言えば競技を通じた報酬はレーティングゲームにおけるプレイヤー報酬です。

# vtuber現象
vtuberは個人のネット上のアバターで、個人が文化的に交換不可能な単位になる現象です。
...
Tenyu基盤ソフトウェアはアバターやゲームを登録できる仕様があり、その構想を採用しています。

# 何が価値を決定しているか
あるいは、なぜ仮想的なアイテムに価値があるか。
価値は人が感じる楽しさであり、価格と区別される。

結論から言えば下記リストの1の仮説に立っています。
仮想的なアイテムに価値があると考えると、外出や物品の消費を伴う趣味と比べて環境負荷を抑えれて、ITの進歩を促進する可能性があり、個人による経済を実現できる可能性があり、社会不適合者の受け皿となる可能性があり、その入手や使用は自宅に居ながらできて手軽であり、副業としてその価値生産に参加できて、経済的文化的可能性が飛躍的に広がり、経済がより複雑化し独創的な価値を生産するようになる。だから**それに価値があるという判断が行われる**。

1. 判断説。それに価値がこれくらいあるとしてみたら自分の人生や世の中がどうなるか、という人間の想像力と判断または社会の複雑性によってそれの価値が決まる。独自の説
2. 労働価値説。一般論
3. 交換価値説。一般論
4. 増幅説。一般論。名前は勝手につけた

1
人は自分が何にどの程度価値を感じるかを意識的に決定できると考える。
価値も価格も誰かの判断や社会の複雑性によって決まる。
社会の複雑性と言っているのは、SNSを通じて話題になっているからとか、最近のニュースとか友人の言動とか、そういったことから影響を受けて価値を判断するという事。
この説は人間の判断によるというある種ずるい論法で、それらが何か根本原理に支配されていると言うような鋭い論理を何も持たない。
これは消極的説明にも思えるが、あらゆる価値や価格を説明できるという利点がある。
しかも社会の複雑性の中で決まるというから、理由を言語化する必要すらない。

ちなみに一例として大抵の国で医者の給料が高い一方でロシアでは低いという事実は、価格を決定する普遍的な原理が存在せず、人間の判断や社会の複雑さで決まっている事を示している。

2
投入された労働量に応じて価値が決定するという説。
しかし、著作権や所有権等が法律によって保護される事で経済が成立していて、明らかに経済を作るための立法が行われているが、では労働価値説に従って法律で既存の商品の生産方法について無駄な手間を強制的にかけさせたら価値が上がるか？
これらの問題を回避するために「最も効率的な生産方法で」とか「法律等によって操作されない自然な経済において」とかいう必要があるだろう。
しかし、窃盗が合法なら取引は必要なく価格は存在しない。1個の主体において窃盗や他者の権利を無視する事は最も効率的な生産方法かもしれない。窃盗をせずに取引をするなら、それは法律によって操作された不自然な経済ではないのか。
私は労働価値説が価値を説明しきれると思わない。

3
使用価値が無い高価な宝石がある一方で使用価値がある水が安価である理由は何かという問題に対する仮説。
交換価値なる概念を持ち出して説明される。
ではブランド品は多くの場合交換を想定されないがなぜ実用性で大差無い他の商品より高いのか？
見栄という社会的実用性があるとでも言うか？
しかし、室内で使うものにもブランド的な価値がある理由は何か？
あるいは、見栄という社会的実用性を信じるなら、CM等を通じてロゴマークを宣伝すると価値が上がるのか？
しかし、そのロゴマークがついた商品は高級らしいと人々に知れ渡る事で見栄が成立して価値が出るというなら、商品に値札を貼ったまま使用すればどうだろうか？
しかし実際には値札を貼ったままにする事で高級であることを誇示しようとする人を見たことが無い。
ブランド品の存在は交換価値説の限界を示している。

4
価値を感じる人が居るから価値を感じる人が居る。
しかし、これは1から説明される。そう判断する人が居るからそう判断する人が居る。
人間の判断が他の人間の判断を追従する場合があるという事であり、増幅説は独自の立場を持たない。
...
逆に現在の状況は、創作活動という現状酷く搾取されているものが法的に保護されたり信頼できるプラットフォームが出現すれば、新たな経済を創出しうる事を示唆します。

一方で、あらゆる知財に適切な対価を支払っていたら今日の産業の大部分が成立しなかったという意見もあります。
私は、そもそも世界的に共有可能な財産である知識を特許とライセンスという2者間の取引で経済化する事に無理があると思います。

パクりの横行は、創作的成果があっても公開しない事に繋がります。
特許の取得は難しいし面倒だし、著作権がカバーする範囲は限られます。
現代社会はまだ創作的成果を扱うための適切な社会制度を持っていないと思います。
...
- Q.新しい価値が生まれるか？  
A.相互評価フローネットワークやオンラインゲーム主軸モデルが生み出す人的動態は新しいものです。  
...
またプレイヤーや配信者やMAD作成者など周辺コンテンツの作成者に報酬を与えれません。他のゲームとの連携も基本的にありません。素材の共有やクリエイターの相互評価プラットフォームの共有もありません。  
さらに、例えばMO型のゲームについてログインせずに今部屋があるのかを簡単に確認する方法がありません。いちいちゲームクライアントを起動して確認する必要があります。過疎が進行したゲームをやりたいと思っても部屋が存在する確率が低くクライアントを起動しなくなります。Tenyuは登録された全てのレーティングゲームについてマッチングの待機人数を提供します。  
さらに、**最も決定的な違いは、従来のオンラインゲームは日本円等の法定通貨の回収モデルを必要としました**。そのためにゲームバランスを崩すような有料アイテムを追加したり、回収できなくなればサービスが終了したり、プレイヤーが多額の金を使って貧しくなりました。多くの場合オンラインゲームの運営はどこかのタイミングでゲームの価値を下げてでも回収に動き出します。プレイヤーは運営が回収に動き出すことを拒む権利を持ちません。運営はプレイヤーがゲーム内に作り上げた資産の価値を自由に制御できます。価値を維持するために有料アイテムが必要な仕様にすることもできます。プレイヤーのゲーム内資産を人質にとるような形で回収を行えます。しかし、本構想は独自の仮想通貨を作成し運営も制作者もプレイヤーも仮想通貨を獲得できて、仮想通貨の価値を高めることで全ての立場が利益を得ます。**本構想は法定通貨を回収するモデルを必要としません**。仮想通貨と法定通貨の交換を個人間で行う事を禁止しませんが、それは本構想で扱わず、外部の交換業者なり個人間の交渉なりで扱われるという想定です。  
...
## FAQ  
- Q.仮想通貨の信頼性は？  
A.大勢の参加者が居てめちゃくちゃな運用がされなければ仮想通貨は価値を持つ。基礎技術と運営者選出の方法の評価、どれだけ参加者が得られるか次第。  
- Q.このプロジェクトを一言で言うと「CPU証明と分散合意の技術を用いて、MMOを軸にした仮想通貨経済圏をつくる」ということですか？  
A.その長さの文で表現するなら、それより適切な表現はそう無いと思います。しかし、仮想経済は仮想通貨に価値を与えるためにあり、最終的に相互評価フローネットワークを通じてソフトウェアの汎用的マネタイズ手段を確立することこそが主旨です。
- Q.技術に関しては、専門家ではないのでよくわかりませんが、このシステムの脆弱性はどこですか？  
A.脆弱性は、専門家が評価して見つからなければ恐らくないだろう、と考えるもので、脆弱性があるかないかを判定する簡単な方法はありません。私は、細かな実装上の問題が入りこむ可能性を除けば、脆弱性が無いと思っています。世界的に利用されているシステムも、専門家が見て脆弱性が無いから恐らく大丈夫と思われてるだけです。  
- Q.ゲームクリエイターがこのプロジェクトに参加するとは、具体的にどういうことでしょうか？  
A.このプロジェクトは、専用サーバー型のゲーム、ホスト型のゲーム、アバターをそれぞれ別個に登録できます。それぞれ仮想通貨の獲得やクリエイター間の相互評価に繋がります。  
- Q.近傍とは？  
A.有向グラフにおいて、あるノードに着目した時、そこからのエッジの先にあるノードが近傍である。つまり矢印の先にあるノードである。P2Pネットワークでは、各コンピューターはある程度の数の他のコンピューターへの参照情報を持ち、参照可能な他のコンピューターに対してエッジを作成していると捉える事で、有向グラフであると捉える事ができる。なおP2Pネットワークと相互評価フローネットワークは全く異なるものであり区別が必要である。両者ともネットワークでありノードやエッジという用語が使用される。なお参照情報とはここでIPアドレスやポートであり、他のコンピューターにアクセスするのに必要な情報である。  
- Q.全体運営者がゲーム運営者の役割をこなすことはありますか？  
A.ありません。全体運営者はゲーム運営者やゲーム自体をBANするなどの権限を持ち監督しうるが、直接ゲーム運営者がやるべきことをやるということはありません。ゲーム運営者が腐敗したら、ゲーム運営者を強制的に他の人に変えて、新たなゲーム運営者によってゲームは健全性を取り戻します。さもなくば、全体運営者によってそのゲーム自体がBANされます。  
- Q.このプロジェクトは、ブロックチェーンに民主主義を導入するってこと？  
A.ブロックチェーンではありません。分散合意とプロセッサ証明は、ブロックチェーンを代替する新しい基礎技術です。民主主義的な側面はあります。従来、オンラインゲームはプレイヤーが構築した仮想価値を無視したアップデートをしたり、サービスを終了しました。しかし共同主体という概念は選挙的プロセスを持ち、参加者の意見を無視した決定が難しくなります。詩的に言えば、共同主体は仮想的価値を守っていく1個の生物のようなものに思えます。  
- Q.運営者が現実で襲われる等してP2Pネットワークが乗っ取られないか？  
A.全体運営者がおかしくなったら、ユーザーはその権限を減らせるし、他の全体運営者を選出できます。  
- Q.乗っ取る用のパソコン（ユーザー）を数多く用意した企業が１人勝ちするとかありません？  
A.この試みが小規模である間、大組織が興味を持つ理由はありません。この試みが大規模になれば、乗っ取りに必要なコンピューターの台数は莫大になります。さらに、乗っ取れても、ユーザーが離れて価値が消失するだけで、価値が得られるわけではないでしょう。そのような方向性で労力を費やすより、正常に参加する方が価値が得られます。  
- Q.運営者がエッジ作成等において、不正とも言い難い微妙なえこひいきをしないか？  
A.エッジ作成は一人一人の公正さに頼っている面がありますが、どのようなエッジを作成したかは公開されます。何の貢献もしてないのにエッジが作成されたとか、重要な貢献をしたのにエッジが作成されないとか弱いエッジしか作成されなかったということがあれば、それは公知されるでしょう。そのようなことは運営者の評判に直接的に響きます。運営者だけでなく、あらゆるエッジ作成者、つまりユーザー全般において同様です。おかしいという評判が立つ事も無いくらいの微妙な匙加減におけるえこひいきは、その程度の事は人類社会において軽度なもので、深刻な理不尽とは言えません。誰もが、やったことなりの評価をそれなりに妥当に受けられるはずです。  
- Q.先行者が得しすぎたりしないか？  
A.現実のビジネスでは、先行者が後発を完全に抑え込む事があります。例えば、工業製品の大量生産では、先行者は少し生産して売って、その利益で設備投資して、という段階的な成長が可能でしょうが、既に大規模な工場を持っている先行者が居る場合、後発は大量生産効果において不利になり、少し生産して売るという事が困難になります。しかし、ソフトウェアは大量生産効果がありません。本構想は、ソフトウェアの共有を推進するのであり、新たに共有されたソフトウェアを使ったソフトウェア開発が常に存在し、先発を軽々と上回る可能性が高い。ソフトウェアでも特定のプラットフォームに依存したコードを生産すると後発のプラットフォームがソフトウェア資産において追いつけず抑え込まれる可能性がありますが、できるだけクロスプラットフォームなコード生産を心がければ防止可能でしょう。  
- Q.マルチ商法じゃないの？  
...
Tenyuの相互評価フローネットワークも無責任支払いがあります。
一人一人のユーザーが他の人の受け取り額を決定しますが、支払いの主体は共同主体であり個々の主体ではないからです。
...
物価下落は初期投資を必要とする起業を困難にする可能性がある。設備や組織や材料を必要とする商売においていきなり高効率な生産をしなければ価格競争力を持てないから。
しかしネット時代における初期投資無しの個人起業という領域が主流になるなら、最低限の生活のコストが安くなる方が起業しやすい。そこでは「当たるか外れるか分からない独創的な取り組み」いわゆる創作活動が要求され、多様な人々が低コストで様々な試みをすべきだからだ。
...
20世紀は工業及び民生品の急速な発達があったが、前述したように人間の構造から可能な販売量に限界があるのでそのような経済成長は終わる。環境負荷の問題もある。
21世紀はオンラインゲームのアイテムによる場所をとらず環境負荷もほぼ無い上限の無い経済成長が実現される。
この観点はこの記事から影響を受けている。
...
# 知識生産経済
私が創作活動による経済とか相互評価フローネットワークとかいっているものは知識生産やソフトウェア生産を効率良く経済化するためのもので、それらTenyuプロジェクトで主張した構想はあらゆる絶望的問題を一般的に解決するための最善の方法を長年模索した結果である。この構想が実現した場合の共同資本の存在意義はそのようなものになる。
相互評価フローネットワークの抽象ノードというアイデアはあらゆる知識生産活動を経済化しうる。そして知識は問題に対する最善の方法を教えてくれる。

# 情報のローカル化現象
Tenyutalkというソフトウェアで追及しているユーザー中心コンピューティングは情報のローカル化を起こすと予想している。
従来ブログ記事などは多くの場合公開されていた。
ツイッターもほとんどが公開アカウントだった。
しかしユーザー中心コンピューティングを追求していくと多くの情報がローカル化するかもしれない。
それはネット炎上や荒らしやデマを防止する可能性がある。
ひとたびネット炎上が起こると反対意見を全て見つけ出し攻撃するような暴徒が大量に現れるが、情報のローカル化はそのような暴徒から守られたローカルなコミュニケーションを育む。

# MMORPGの限界と対策
MMORPGはプレイヤーに成長を感じさせる必要がある。
そのため、諦めてしまうほど困難でもなく、ワンパターンでクリアできてしまうほど簡単でもないゲームを提供する必要がある。

従来どのような方法が用いられたか。
- ひたすらレベルキャップを上げたり上位装備を実装し続ける。ほとんどのMMORPGはこの対策を採用し、クライアントソフトウェアの肥大化や仕様の過剰な複雑化を招いた。新規と古参の差が広がり続けてやがて新規が来なくなる。
...
私が予想するこの問題に対する他の対策
- 制作者になる。自ら新しいゲームを作る。これも一種の成長。
- 人間関係の構築を楽しむ。様々なオンラインゲームにおいて一貫して共通のアバターを用いる。ネトゲがvtuberに対応する。

Tenyuの連合型MMO構想はこれらのアイデアを採用しています。
