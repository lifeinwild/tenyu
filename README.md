...
例えばTenyuLicenseは著作物の利用者に相互評価フローネットワーク上で[公正なエッジ作成](#作者権)を行う事を要求します。  
...
MMOの重要な特徴は巨大なRMT市場を実現したことです。**MMOは[仮想経済](#仮想経済)です**。  
...
Tenyuでは[相互評価フローネットワーク](#相互評価フローネットワーク)で分配される仮想通貨に価値を持たせる役割があります。
...
[従来のMMOは持続可能なモデルではありませんでした](#ンテンツ消費問題)。MMOはいずれ廃れてしまいます。廃れてしまえば蓄積されたゲーム内資産は価値を失います。既に多くのMMOプレイヤーがそれを学習しました。  
...
全体運営者はゲーム部分だけでなく[相互評価フローネットワーク](#相互評価フローネットワーク)を通じたソフトウェア制作の汎用的な収益化も実現する必要があります。
...
私が[分散合意](#分散合意)と呼んでいるものは、近傍との[局所的多数決](#局所的多数決)を全ノードが一斉に繰り返す事で全体で多数決をした場合と同じ結果が各ノードの元に現れるというP2Pネットワークの現象です。
...
[信用はただ自分の元で存在する](#主観的な信用)ということです。それで信用で投票権が差別化された局所的多数決が可能です。
そして分散合意によって[全体運営者](#全体運営者)の選出投票など、ネットワーク全体での意思決定が可能になります。  

つまりプロセッサ証明の「近傍ノードの信用しか算出できない」という欠点と「近傍についてダミーノードを排除できる」という長所と、分散合意の「ダミーノードに弱い」という欠点と「局所的多数決（近傍とのやり取り）のみで良い」という長所が相補的に組み合わさり、P2Pネットワーク全体での多数決（[客観](#客観)DBのハッシュ値や次の全体運営者について多数決する）が実現されます。  

多数決が実現されるとは即ち少数派意見が排除されるということであり、「悪意あるノードは少数派である」という前提が成立するなら、不正な改ざんが阻止されるということです。

## 選挙
選挙は[プロセッサ証明と分散合意](#分散合意とプロセッサ証明の相補性)によって実現され、[全体運営者](#全体運営者)の選出や任意の内容のアンケート等ができます。これらP2P技術によって、選挙を不正に操作する事は技術的にできません。[同調処理](#同調処理)と異なり**全ノードが同時に一斉に行います**。さらに、開始時点で各ノードで主張値がまちまちという前提です。 
PowerVoteがその実装です。
...
[客観](#客観)DBを全ノードで同値にするための処理で、随時、**各ノード各々のタイミングで**行われます。この点で[選挙](#選挙)と異なります。同調処理の[分散合意](#分散合意)は異常排除型でありターン概念が無いという事です。
