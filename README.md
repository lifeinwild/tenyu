...
MMORPGはプレイヤーに成長を感じさせる必要があります。
従来どのような方法が用いられたか。
- ひたすらレベルキャップを上げたり上位装備を実装し続ける。ほとんどのMMORPGはこの対策を採用し、クライアントソフトウェアの肥大化や仕様の過剰な複雑化を招いた。新規と古参の差が広がり続けてやがて新規が来なくなる。コンテンツ消費型。
...
Tenyuの連合型MMOが示す新たな解決策
- 制作者になり自分のゲームを登録する。自ら新しいゲームを作る。これも一種の成長。
...
## 全体的なイメージ
各ノードは**普段**近傍と客観DBについて**各々のタイミングで随時**[同調処理](#同調処理)しています。
...
選挙型の分散合意においてターン概念が必要な理由は他のノードの処理を待つ必要があるからです。各ノードが他のノードを待たずに最速で実行するようでは例えば「遅いノードが3ターン目に入った時速いノードは既に全20ターンを終えて局所的多数決を締め切っている」という問題が生じて値が収束しないからです。  

ところが同調処理等の異常排除型の分散合意ではターン概念が不要になります。  
異常排除型では「正常動作していれば当然どのノードも正しい値を持っている」という前提があり、僅かな異常値を排除するという処理になるので、選挙型とは前提が違います。選挙は最初各々のノードで主張値が全く違って当然です。選挙型では攻撃者が関与せずとも開始時点ではノード毎にまちまちな値を主張します。同調処理では客観DBの整合性を扱うのであり、ほとんどのノードが正しく一斉更新を重ねていれば開始時点でも正しい値を持っていて当然なので、この場合ターン概念が無くとも性能及びセキュリティが成立します。  

## 一斉更新
一斉更新は客観DBの同調処理下（統一処理下といっていたこともあった）における客観DBの高速な更新方法です。客観DBは主に一斉更新によって更新されます。

同調処理によって全ノードで客観DBが同値に保たれますが、逆にそのせいで一部のノードが客観DBを更新するだけでは即座に古い値に巻き戻ります。だから客観DBの更新は「一斉更新」が必要になります。一斉に客観DBを更新すれば巻き戻りません。  

そしてもう1つの要点は、**「客観DBの内容である重要な承認情報は大部分が誰かユーザーの電子署名を根拠に作成・更新された情報だ」**というものです。例えば仮想通貨の送金は送金者の電子署名があれば堅牢という事になります。プロフィールの更新はそのユーザーの署名があれば堅牢という事になります。つまり本人または特権を担当している者の電子署名がついたメッセージを受け取ると各ノードはそれを受け入れて客観を更新します。  
...
一斉更新ではある程度メッセージを溜め込んでから（UserMessageList）一斉に反映します。反映タイミングは全ノード同時でなければならず、実際2分に1回しかありません。さらにメッセージリストは全ノードで同値である必要があります。メッセージが一部のノードにしか届いていないという事があってはならないという事です。このメッセージ拡散の問題の解決策はメッセージ受付サーバかブロードキャスト方式か2通り考えられましたが現状では前者を採用しています。  
...
## 同調処理
客観DBを全ノードで同値にするための処理で、随時、**各ノード各々のタイミングで**行われます。この点で選挙と異なります。同調処理の分散合意は異常排除型でありターン概念が無いという事です。
  
なお同調処理を通じて客観が更新される（近傍と同調される）のは補助的な処理であり、客観は主に一斉更新によって更新されます。同調処理の存在意義はオフライン状態から復帰したノードが遅れを取り戻すためにあります。ネットワーク全体が何らかの攻撃に晒されて客観DBの不整合が大規模に生じた場合でも同調処理によってP2Pネットワーク全体で客観DBが同値になります。
