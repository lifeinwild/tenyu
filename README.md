...
多くの場合常駐空間では明確な勝利条件が無く様々な活動が可能です。典型的には常駐空間ゲームとは**PvE=環境との戦い**です。そのような環境において他のプレイヤーの存在は環境の一部です。プレイヤー動態に依存してその時々の状況が存在し、その中で何らかの条件を達成すると[材料](#材料)がもらえます。  
...
レーティングゲームの空間はその試合専用のインスタンス空間で、試合が終わったら削除されます。サーバが無いのでP2Pネットワークに記録される情報とプレイヤーのPCに保存される設定等を除けば独自のデータを蓄積していく事はできません。
