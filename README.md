...
達成されている重要な性質。
- 各ノード毎に動的に問題が作られる事で、問題の作成タイミング及び回答締め切りまでの時間をソフトウェア側で制御できて、実際8時間おき2分間しか演算量証明しないで済む。従来技術と比較して劇的に省電力かつ他のアプリ（ブラウザやゲーム）の動作を妨げない。
...
A.ブロックチェーンではありません。分散合意とプロセッサ証明は、ブロックチェーンを代替する新しいP2P技術です。民主主義的な側面はあります。  
- Q.運営者が現実で襲われる等してP2Pネットワークが乗っ取られないか？  
A.全体運営者は複数居てそこでも投票があります。全体運営者がおかしくなったら、ユーザーはその権限を減らせるし、他の全体運営者を選出できます。  
- Q.乗っ取る用のパソコン（ユーザー）を数多く用意した企業が１人勝ちするとかありません？  
A.この試みが小規模である間、大組織が興味を持つ理由はありません。この試みが大規模になれば、乗っ取りに必要なコンピューターパワーは莫大になります。さらに、乗っ取れてもユーザーが離れて価値が消失するだけで、価値を奪えるわけではないでしょう。そのような方向性で労力を費やすより正常に参加する方が価値が得られます。  
- Q.運営者がエッジ作成等において、不正とも言い難い微妙なえこひいきをしないか？  
A.エッジ作成は一人一人の公正さに頼っている面がありますが、どのようなエッジを作成したかは公開されます。何の貢献もしてないのにエッジが作成されたとか、重要な貢献をしたのにエッジが作成されないとか弱いエッジしか作成されなかったということがあれば、それは糾弾されるでしょう。そのようなことは運営者の評判に直接的に響きます。運営者だけでなく、あらゆるエッジ作成者、つまりユーザー全般において同様です。おかしいという評判が立つ事も無いくらいの微妙な匙加減におけるえこひいきは、その程度の事は人類社会において軽度なもので、深刻な理不尽とは言えません。誰もがやったことなりの評価をそれなりに妥当に受けられるはずです。  
- Q.先行者が得しすぎたりしないか？  
A.現実のビジネスでは、先行者が後発を完全に抑え込む事があります。例えば、工業製品の大量生産では、先行者は少し生産して売って、その利益で設備投資して、という段階的な成長が可能でしょうが、既に大規模な工場を持っている先行者が居る場合、後発は大量生産効果において不利になり、少し生産して売るという事が困難になります。しかし、ソフトウェアは大量生産効果がありません。本構想は、ソフトウェアの共有を推進するのであり、新たに共有されたソフトウェアを使ったソフトウェア開発が常に存在し、後発のソフトウェアは先発のを軽々と上回る可能性が高い。ソフトウェアでも特定のプラットフォームに依存したコードを生産すると後発のプラットフォームがソフトウェア資産において追いつけず抑え込まれる可能性がありますが、できるだけクロスプラットフォームなコード生産を心がければ防止可能でしょう。  
- Q.マルチ商法じゃないの？  
...
20世紀は工業及び民生品の急速な発達があったが、前述したように可能な販売量に限界があるのでそのような経済成長は終わる。環境負荷の問題もある。
