...
私はここに示した構想によってそのような問題が根本的に解決すると思っています。  
...
有料化する事無く＝日本円の徴収無く  
自由な相互利用を保ちながら＝作者権やTenyuLicenseで説明される最小限の保護のみで  
幅広くネット上での活躍が収益化される状況＝相互評価フローネットワークを通じた仮想通貨分配システム  
という事です。  
入手した仮想通貨は誰にでも送金できるので法定通貨と交換する事も可能です。  
...
さらに、従来の著作権や特許は世界の生産性を少なくとも部分的に阻害していました。他人の創作的成果を自由に利用できなかったからです。著作権や特許の制度は、部分的に生産性を阻害する面があったとしても全体として利益があるという政治的判断の元で存在していると思います。
しかし作者権が主張する事は誰が作者または貢献者であるかを明示するという事であり、それは**部分的な阻害すらありません**。
...
より厳密には次数補正というアイデアがありエッジを経た回数で補正がかかります。次数補正の係数cは全体運営者が設定できて、相互評価フローネットワーク全体で一定です。例えばエッジを経るたびに1.05がかけられるとすると、より根本から離れた、共同主体から離れたノードが収益化されやすくなります。
![Alt text](readmeImg/edges6.png)  
全員がエッジ作成活動をすると、全体として複雑なネットワークができます。WEBページはその制作者や素材提供者にエッジを作成し、ゲームは衛星作品や運営者や創作者にエッジを作成し、創作者は影響を受けた講座サイトや他作品のWEBページにエッジを作成します。総じて言えば、オンライン上の貢献関係についてエッジを作成していくわけです。緑の旗は後述する共同主体を意味します。共同主体からフローが湧き出て、そのフローの到達量に応じて仮想通貨が分配されます。黄色の旗はオンラインゲーム、動物の顔はユーザー、GUIのようなものはWebページです。  
...
## ノード、エッジという言葉  
P2Pネットワークのノードやエッジについて言っている場合と、相互評価フローネットワークのノードやエッジについて言っている場合があるので、混同しないよう注意してください。P2Pノード、P2Pエッジと言っていたら確実にP2Pネットワークの事を言っています。P2Pネットワークはごく標準的な意味のP2Pネットワークですが、相互評価フローネットワークはP2Pネットワークで共有されるDB上のデータです。P2Pネットワークの全貌はどのノードも知りませんが、相互評価フローネットワークの全貌はどのノードも知っています。  
...
だから現在の状況は創作活動という現状酷く搾取されているものが法的に保護されたり信頼できるプラットフォームが出現すれば新たな経済を創出しうる事を示唆します。
...
# 知識生産経済
私が創作活動による経済とか相互評価フローネットワークとかいっているものは知識生産やソフトウェア生産を効率良く経済化するためのもので、それらTenyuプロジェクトで主張した構想はあらゆる絶望的問題を一般的に解決するための最善の方法を長年模索した結果です。私の直観によれば、この構想が実現した場合の共同資本の存在意義はそのようなものになります。
