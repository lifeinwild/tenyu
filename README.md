...
## １シナリオ
Tenyuは連合型MMOを実現するので様々なゲームが登録されますが、1つレーティングゲームが登録されたとします。ゲームAとします。  
ゲームAの周辺にはその制作者、使用されたライブラリ及びライブラリの制作者、宣伝した者等が存在します。  
その場合このようなエッジが作成されます。  
ゲームA→ゲームAの制作者  
...
## ノード、エッジという言葉  
P2Pネットワークのノードやエッジについて言っている場合と、相互評価フローネットワークのノードやエッジについて言っている場合があるので、混同しないよう注意してください。相互評価フローネットワークはP2P技術と無関係です。P2Pノード、P2Pエッジと言っていたら確実にP2Pネットワークの事を言っています。P2Pネットワークはごく標準的な意味のP2Pネットワークですが、相互評価フローネットワークはP2Pネットワークで共有されるDB上のデータです。P2Pネットワークの全貌はどのノードも知りませんが、相互評価フローネットワークの全貌はどのノードも知っています。  
...
つまり、全体運営者が居ないし、仮想通貨残高など価値ある承認情報が一切ありません。
...
以下、各ノードが互いの信用値をどのように高めていくかという事、それでなぜ全体が堅牢になるかを説明します。

A。基盤ソフトウェアはプロセッサ証明を実施する日時を定数として持っています。その日時が来ると全ノードは一斉に近傍にランダム値を含む問題作成情報を送ります。問題作成情報は日時や作成者やランダム値が含まれています。
...
受け取った全問題作成情報からハッシュ値を作り、ハッシュ値から一意に定まる動的な問題関数を作成します。
このinteractionメソッドは近傍から多数の問題作成情報を受け取り、それらを総合して1つの問題作成情報を作成し、139 CPUProvement.parallelSolveから呼び出されるCPUProvementクラスは問題作成情報からハッシュ値を作成してハッシュ値から動的にプログラムを作成し、そのプログラムの返値が特定の値になる引数を探索します。その探索された引数が回答情報になります。
...
このとき全問題作成情報に任意の値を加える事ができます。結局ハッシュ値を作れればいいのと、問題作成情報のランダム値に依存してさえいれば有効であり、他の情報に依存してはならないという制約はないからです。
例えばコード中でparallelNumberとされていますが、近傍からただ1つランダム値を受け取るだけで、そこに勝手に番号を加えてハッシュ値を変化させ、各ハッシュ値から作成された問題の回答を計算しまとめて回答する事で信用を大幅に高める事ができます。問題作成情報に問題の番号を回答者側で勝手に加える事で複数の問題を作成できるということです。
この仕様によって、計算能力に余裕があるノードは多数の問題を解いてその分多くの投票権を獲得できます。
...
C。問題関数はAにおいてランダム値に依存している事から予め作成しておく事ができません。即ち予め回答を計算しておく事もできません。
そして問題を解いたら近傍に回答として送信します。このとき回答に全問題作成情報を含めます。
その近傍が送ってきた問題作成情報以外の問題作成情報も全て含めて送信するという事です。
回答を受け取ったノードは、その全問題作成情報の中に自分が送ったランダム値が含まれている事を確認してその時回答が計算された事を確認し、
...
するとダミーアカウントを排除できます。1台のPCで大量のダミーノードを実行してもプロセッサ性能が限られているのでこのような方法の元で他のノードから信用値を獲得する事はほとんどできません。
...
過少に見せるとは即ち「多数のノードに対してまとめて回答を作成する」という有利さを捨てている事を意味します。近傍数を半分に見せようとすれば全ランダム値を半分に切断する事になり、検証者は自分が送信したランダム値が含まれているかを確認するので、回答者は切断された2つのランダム値それぞれに対して回答を計算する必要があります。
...
ところで、あるノードの元で他のノードがプロセッサを持っている事について信用されたとして、
それはあくまでそのノードの元でのみ有効な信用であり、その信用を他のノードに伝播する術はありません。
「あのノードはこれくらい信用できるよ/プロセッサ性能を伴っているよ」と言っても他のノードは信用しないという事です。信用の伝播は生じません。
では全ノードが他の全ノードの信用を自力で確認する必要があるかというと、そうではありません。
分散合意というアイデアにおいてネットでの投票を実現するにあたり信用を伝播する必要はありません。

分散合意とは、近傍との局所的多数決を全ノードが一斉に繰り返す事で、1か所に集計した場合と同じ結果になるというP2Pネットワークの現象です。
局所的多数決をするだけで良いから信用を伝播する必要がありません。
信用はただ自分の元で存在し、近傍に対してだけ算出されればいい。それで局所的多数決が可能です。
そして全体運営者投票など、ネットワーク全体での意思決定が可能になります。
つまりプロセッサ証明の「近傍ノードの信用しか算出できない」「近傍についてダミーノードを排除できる」という欠点と、分散合意の「局所的多数決のみで全体の多数決をした場合と同じ結果になる」「ダミーノードに弱い」という性質が相補的に組み合わさり、P2Pネットワーク全体での多数決が実現されます。
多数決が実現されるとは即ち少数派意見が排除されるということであり、一部のノードによる不正な改ざんが阻止されるということです。例えばP2Pネットワークで共有されるDBのハッシュ値とか、全体運営者が誰であるかという情報について過半数のノードが正しく知っている限り改竄されることがありません。

そうして全体運営者を分散合意によって選出します。この選出過程は堅牢であり改竄できません。選出されたノードは自動的にいくつかの権利が与えられ、P2Pネットワーク上で特別な役割を果たせるようになります。**投票によって選出された人が決定した事だからという理由で各ノードはその決定に従います**。全てを分散合意で決定するのはユーザーの操作が大変ですが、特別な権限を持つノードを選出する事で大抵の役割をそのノードに任せる事ができます。
