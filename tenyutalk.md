...
# 参照
Tenyuは様々なオブジェクト（User等）を扱いますが、それらオブジェクトは様々な場面で参照できます。

ユーザーがアップロードした[成果物](#成果物)も参照できます。その参照を通じてそのファイルやフォルダをDLします。

[参照はURL表現を作成できてMarkdownの記事上でハイパーリンクとして使用できます](https://github.com/lifeinwild/tenyu/blob/master/rpc.md)。参照は参照先オブジェクトのmedia typeを持ち、一部のmedia typeではMarkdown記事上で自動的に参照先が取得されて表示されます。ちょうどHTMLのIMGタグのように。

以下の仕組みによって参照先オブジェクトを管理しているノードがオフラインになっても多くの場合参照できます。
- 参照は参照先オブジェクトをミラーしているノードリスト等の情報を持ちアップロード者ノードがオフラインならミラーノードに問い合わせます。
- アップロード者ノードも[ミラーノード](#ミラーノード)もオフラインの場合、近傍に問い合わせます。
- 一度取得したオブジェクトは自分のノードで[キャッシュ](#キャッシュ)されます。

参照先がフォルダで表現されている成果物だったらそのフォルダ以下の全ファイルフォルダがDLされます。

## ミラーノード
リポジトリをミラーするノード。
そのリポジトリ内の全アーティファクトの全バージョンをコピーする。

## キャッシュ
特定のアーティファクトの特定のバージョンをキャッシュしておき、再びアクセスする時キャッシュから探す。

## フレキシブル参照
同じmajorバージョンの最新の成果物を参照したり、majorも無視して最新版を参照できます。

## セキュア参照
特定のバージョンの成果物を参照します。さらに参照が参照先のハッシュ値を持ち照合します。
