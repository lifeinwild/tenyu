...
特殊なアイデアはなく、複数の簡単な選択の組み合わせ。
アプリ名によるURLや検証アーキテクチャはちょっとしたアイデアと言える。しかし検証アーキテクチャはRPCに限った事ではなく情報処理システム一般のアイデア。
...
だからクラスベース型の言語で記述されたシステムではクラス名を含める事は重要になる。

しかしクラス名を含めるRPCは事前にクラス定義を共有する必要が生じる。**この点がTenyutalkならアプレット構想やWWWを置き換える事を設計目標としている事からSOMのハイパーリンクはTenyutalk上で作成されると言えるので解決される**。
...
- 状態検証。悪意の元調整された状態を持っていると深刻な動作を引き起こす可能性がある。
