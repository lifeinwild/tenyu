# 概要
これは私がTenyuプロジェクトのソフトウェア構成（どの言語、フレームワークを使うか等）の判断を説明するための文書です。Tenyuプロジェクトに興味を持つ人が居たとしてそのソフトウェア構成に同意できるかが1つのハードルになります。だから説明する必要があります。
その他雑多なメモも含まれます。

私がこの文書で考えているのは**新しいコンピューターやOSの出現を促すには世界的にソフトウェア資産をどう整備すべきか**であり、そのためにここに書かれた考え方が世界中のソフトウェア制作者に広まり同意を得る必要があります。  

当然私はこれらの考えに同意する一人であり、Tenyuプロジェクトのソフトウェア構成はその観点から判断しました。私はTenyuプロジェクトのソフトウェア構成についてまだ実現されていないいくつかの潜在的可能性も考慮して判断しました。

クロスプラットフォームを超えたバーチャルプラットフォームという語の定義（概念自体はpure javaと同じ）、
...
# バーチャルプラットフォーム(pure java)    語の追加、整理
