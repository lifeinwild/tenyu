...
私がこの文書で考えているのは**新しいコンピューターやOSの出現を促すよう世界的にソフトウェア資産を整備するにはソフトウェアエコシステムはどうあるべきか**であり、Tenyuプロジェクトのソフトウェア構成はそのように判断しました。
...
私はTenyuプロジェクトのソフトウェア構成について、まだ実現されていないいくつかの潜在的可能性も考慮材料にして判断しました。
...
バーチャルプラットフォームという語はより厳しい条件を持ち、例えばpure javaなど、完全に仮想環境のみに依存している事を要求します。とはいえjava以外でもバーチャルプラットフォームなソフトウェアはありうるので、pure javaという語はjavaコードにしか使えないのでバーチャルプラットフォームという語を定義しました。
