...
Glbはいくつかsetup系メソッドを持ち、setup系メソッドを通じてメンバーを初期化します。
...
Glbはglb.Glbという名前で作成されるので、同じパッケージに置いて同じ名前のインターフェースを持たせる事で動作させれる場合もあるかもしれませんが、そのインターフェースの実装がまた他の様々なものに依存している場合、難しくなります。
...
結論から言うと継承とユーザーコードによる構成はどちらも問題がありました。そして最終的には[継承構造](#継承構造_案_解釈)を肯定的に捉えるという事になりました。
