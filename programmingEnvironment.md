...
例えばライブラリのコードに問題があるからと独自にリフレクション等を駆使して解決が試みられる場合があるが、大元のプロジェクトへの修正と高速リリースによって解決されるべき場合が多い。とはいえ、自分のケースだけで解決するよりもより一般的な解決をする方が難しいタスクになる。なぜ他のプロジェクトのために時間を使わなければならないかという問題になる。

このような問題も相互評価フローネットワークがあれば柔軟に対応できる可能性がある。
