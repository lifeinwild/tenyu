# 概要
これは私がTenyuプロジェクトのソフトウェア構成（どの言語、フレームワークを使うか等）の判断を説明するための文書です。Tenyuプロジェクトに興味を持つ人達が居たとしてそのソフトウェア構成に同意できずに参加しないかもしれません。だから説明する必要があります。
