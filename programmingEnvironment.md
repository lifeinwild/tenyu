...
例えば、**新しいコンピューターやOSの出現を促すために世界的にさらにクロスプラットフォームを追求すべきだ**という考えが強くあり、
この文書はその観点が強く出ています。
...
構成的多重継承は継承が抱える全ての問題を解決しないかもしれない。
compositionalクラス間の依存ができないから。
class NormalClassA extends CompositionalClass1, CompositionalClass2  
ここでCompositionalClass1とCompositionalClass2は互いに依存できない。
この点で従来継承で解決されていた問題の一部が構成的多重継承では解決できないという事になるし、compositionalクラスで表現されるべき概念は根本的性質のようなものであるべきで他の性質との相互依存は子クラスで解決されなければならない。

言い換えれば構成の各要素間で相互依存できないという事で、[相互依存をサポートするアイデア](#構成を言語仕様でサポートする_parent_素案)より劣ります。

しかし、もしかしたらその制限は実際問題が無いかもしれません。「ある種の概念はcompositionalクラスでうまく記述できて、それらの概念さえ記述できれば継承に他の問題は残らない」という可能性もありそうな気がします。

「このクラスを継承するクラスはこのクラスも多重継承していなければならない」というような制約を導入すればcompositionalクラス間の依存もできそうです。  
子クラスが実装していなければならないインターフェースや継承していなければならないクラスについての制約を記述すれば、そのインターフェースやクラスが必ずあるものとして扱えるという事です。
...
- [Glbは各モジュールの相互参照をうまく扱えている。Glbは構成的。](#Glb_案)じゃあもっと粒度の小さいGlbみたいなものを扱おう。
