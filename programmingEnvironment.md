...
例えばJava AppletはJava11で廃止されました。これまでいくつものAPIがdeprecatedないし廃止されました。Python 3.0は後方互換性を捨てました。このようなリスクを考えるとある時点で自分のプロジェクトに必要な機能がすべてあるというだけでは言語を採用するのに不十分で、その言語が長期的にどのような方向性で変化していくのかを考える必要があります。つまり**言語が宣言していることや期待されていること**が重要です。
...
- [カスタム権限](https://docs.oracle.com/javase/tutorial/security/userperm/index.html)  
  カスタム権限にメンバー変数を持たせてPermission#implies()をオーバーライドすれば大抵の要求に対応出来ると思う。文脈中の権限数が増えすぎるとパフォーマンスの問題が生じる可能性があるので、広い範囲を許可する権限を少し付与して、セキュリティが問われる各メソッドは狭い範囲の権限を要求してimpliesでtrueにするというような方法で文脈中の権限数を低く抑えるべきだろう。
- 権限付与  
...
つまりJavaでは各クラス毎に権限が付与されているが、加えて、スレッドの経路上の全クラスでその権限が付与されている必要がある。そして何らかの権限を要求するコードがアクセス拒否するかが決まる。

ただしdoPrivilegedを使うと、”今そのコードを実行しているスレッドの経路上の全クラスがその権限を持っている必要がある”という制限を無視して、そのクラスに本来与えられた権限だけでアクセス拒否を判定する。

ポリシーファイルに思った問題点。ポリシーファイルはデフォルトの位置か起動時のコマンド引数で指定した位置に置いておく必要があり、programmaticallyに与えれるものではないようだ。そのせいで特定タイミングでだけあれば良い権限を動作中ずっと与えておく事になる。

細かな調整はSecurityManagerを継承したクラスを作り一部メソッドをオーバーライドして対応できるようだが、性能が悪化しやすいので注意する必要がある。

Javaでは同じクラスであってもクラスローダー毎に静的フィールドが作られるが、ではクラスローダー毎にSecurityManagerをセットできるか？
クラスローダーの委任モデルによって上位のローダーでロードされるSystemクラスは子ローダーでロードしえない（委任される）ので、そのフィールドsecurity（getSecurityManager()の返値）がクラスローダー毎に作られるという事はない。  
https://coderanch.com/t/385932/java/ClassLoader-Static-variables  
Javaのこの動作を確認するコードを書くときは委任モデルに注意する必要がある。デフォルトのクラスローダーからTargetクラス（複数ローダーでロードされるクラス）が見えてしまっていると、ロード済みなら委任されるので。
