...
例えば、**新しいコンピューターやOSの出現を促すために世界的にさらに[クロスプラットフォームを追求すべき](#バーチャルプラットフォーム_語の追加_整理)だ**という考えが強くあり、その考えが判断に影響しています。この文書はその観点が強く出ています。特に重要なアイデアは[fat_java_案](#fat_java_案)または[実行時依存性解決](#実行時依存性解決_案)です。
...
- [クラスとは_説明](#クラスとは_説明)
- [継承構造_案_解釈](#継承構造_案_解釈)
- [１個の情報処理システムの限界](#１個の情報処理システムの限界)
- [継承より構成_比較_考察](#継承より構成_比較_考察)
- [問題は継承ではなく抽象的設計そのもので、しかもそれは本質的難しさではないか？_仮説](#問題は継承ではなく抽象的設計そのものでしかもそれは本質的難しさではないか_仮説)
- [構成的多重継承_案](#構成的多重継承_案)
- [構成を言語仕様でサポートする_parent_案](#構成を言語仕様でサポートする_parent_案)
...
そして**バーチャルプラットフォームは仮想環境にのみ依存しているという条件を指し**、その仮想環境さえ提供されればどんな環境でも動作するので、その意味で**未知の**（将来的に出現する）プラットフォームでも動作させやすい（ランタイムさえ提供されれば動く）性質があります。

クロスプラットフォームなソフトウェアは環境毎のネイティブコードを含み環境に応じて切り替えて使用している場合があり、**未知の環境で動作させれない**場合があります。バーチャルプラットフォームなソフトウェアはそのようなものを含みません。

さらにクロスプラットフォームなソフトウェアはソースコード互換しかない場合があります。Cでifdefを多用してターゲット環境別にビルドできるようにしている場合など。バーチャルプラットフォームなソフトウェアはバイナリ互換が必須です。

そのため、私はクロスプラットフォームという語に不満があり、バーチャルプラットフォームという語を勝手に追加し、バーチャルプラットフォームなソフトウェアを増やすべきと考えています。
...
例えば誰かが新しいプラットフォーム（命令セットやOS)を作るとします。新しいプラットフォームはソフトウェア資産において古いプラットフォームに負ける可能性が高いですが、もしバーチャルプラットフォームなソフトウェア資産が大量にあれば、ランタイムさえ実装すればソフトウェア資産の面で追いつけます。
...
- 第三者による実装がライセンス上可能である事。新しいプラットフォーム（CPU)を作る人がそのプラットフォーム版のランタイムを自由に作成して配布できる事。
...
ライブラリでネイティブコードを同梱しているものを広く調査して、JDKにそれらネイティブコードを代替するAPIを実装する。
...
Cの主な領域はOS、デバドラ、組み込みであり、そのような領域ではCの言語仕様がハードウェア（CPU)を詳細に制御するために適しています。書いたことしか起きない、実行時間が予測可能である事が重要とされます（抽象的な言語はその点で逆効果）。他に実行ファイルが小さい事や省メモリである事も要求されます。これらの条件をすべてクリアできる言語がCのようです。
...
AIによるデバッグというアイデアもあるようで、言語の代替以外でCへの不満が解決される可能性もあります。

なんにせよCが置き換えられる可能性は現状全く見当たりません。Rustが少しそのような期待を受けましたが、フリースタンディングな領域ではポインタ変数を定数や変数等で初期化する場合があり、そのようなものは静的にポインタの安全性を判定できないのでunsafeであり、unsafeなコードが増えるようではRustを使う意味がどの程度あるのかという事になります。しかしMSがC言語の代替として[Rust系のアイデア](https://github.com/microsoft/verona/blob/master/docs/faq.md)を検討したようです。
...
言語はエコシステムからの期待に応じてアップデートされます。そうしなければエコシステムと軋轢が生じます。だから新しい長期プロジェクトが言語を選定する時、その言語への期待がどんなものかを調べるのは合理的です。言語がある時点で自分のプロジェクトに適した特徴を持っていても、その特徴が長期的にサポートされていくか？突然破棄されないか？という問題があります。あるいはある時点で必要な特徴が無くてもいずれ実装されると見込める場合もあります。

例えばFortranは非常に古い言語で、科学技術計算の期待に応え続けてきたという事になると思いますが、**時代によって具体的な要求は様々だった**ようです。言語はエコシステムからの期待に応えるためにアップデートされ、変化し続けます。

言語は一部の期待を捨て去る場合があります。
近年のメジャー言語でも後方互換性が捨てられた場合があります。
例えばJava AppletはJava11で廃止されました。これまでいくつものAPIがdeprecatedないし廃止されました。Python 3.0は後方互換性を捨てました。このようなリスクを考えるとある時点で自分のプロジェクトに必要な機能がすべてあるというだけでは言語を採用するのに不十分で、その言語が長期的にどのような方向性で変化していくのかを考える必要があります。つまり**その言語が現時点で何を実装しているかよりもその言語にどのような期待が集まっているか**が重要です。

言語が置き換えられる事はほとんどありませんが、シェアを奪いあう面はありそうです。
例えば、JavaとC#はシェアを奪い合っているように見えます。特に2014年（Java8の年）。  
https://www.tiobe.com/tiobe-index/java/  
https://www.tiobe.com/tiobe-index/csharp/  
つまり、**技術的に似ている言語では容易にユーザーの移動が起きます**。
**言語というよりもパラダイムがユーザーを得ている**と言えそうです。
...
- JVMアプリ
    ...
    汎用目的◎  
    多言語〇  
    もしWEBアプリの代わりにJVMアプリが流行ったとすると、即座にセキュリティが問題になります。WEBのようにオンラインで入手したコードをサンドボックス実行する普及した仕組みが無いからです。ただしSecurityManager、AccessController、カスタムPermission等を使ってWEBのようなオンラインからのコードのDL及びサンドボックス実行に耐えるシステムを構築可能です。
...
javaはnull安全がありませんが例外処理や静的解析ツールで補えます。
...
さらに、ハードウェアに近い領域では抽象的設計の上ではコードがどのようなハードウェアの動作を引き起こすのかイメージしづらく、抽象的設計は好まれないようです。  
主要OSはほとんどCで書かれているようです。Linuxは一度C++に移行しようとしてまたCに戻ったそうです。Cはクラス概念を持っていません。さらにハードウェアも予測不可能な根本的変化がありえる対象で、抽象的設計はリスクがあります。例えばモバイルは消費電力に敏感で、memristorはメモリがプロセッサになります。
...
- C++プログラマーは抽象的設計と低水準操作を両方扱います。扱う概念が多岐に渡るのでC++プログラマーは均質性がありません。
...
- C++
    ...
    **1つ恐ろしい可能性がある**。C++はWebAssemblyでWebフロントエンドにも乗り込めるし、WASIプロジェクトの結果次第ではVMすら手に入れてバイナリ互換のクロスプラットフォームすら手に入れる。そこまでいくとJava+Cの対応可能範囲に追いつく。
    多くの重要なプログラムがC++で作られている。JDKや.NET Coreや主要ブラウザ等。
- Java+C
...
- 特殊なハードウェアを多用するシステム（軍事システム等）ではJNI連携が多発するので、Javaは適さないか何かソリューションが必要。

当初私はプログラミング言語の未来についてこう考えていました。
[バーチャルプラットフォームなソフトウェア](#バーチャルプラットフォーム_語の追加_整理)が増える事によってプラットフォームの進歩が激化したり多様化すればC++等のネイティブコンパイル言語のソフトウェア資産はついてこれないと。
...
# 実行時依存性解決_案
ビルド時に依存性解決するのではなく実行時にしようというアイデアです。さらにリポジトリをエンドユーザー環境に構築すれば配布物がライブラリを同梱しなくなり小さくなります。
...
- OS側JDK
- APIを強化し[fat java案](#fat_java_案)や[JNI用実行時コンパイル案](#JNI用実行時コンパイル_案)を採用し[バーチャルプラットフォーム](#バーチャルプラットフォーム_語の追加_整理)を追求したJDK
- [実行時依存性解決](#実行時依存性解決_案)
...
SmalltalkのクラスライブラリにおけるOOPのデザインパターンが他の言語のクラスライブラリ等に影響を与えた事は確実だろうと思います。
...
[継承は使いどころが難しい](#問題は継承ではなく抽象的設計そのもので、しかもそれは本質的難しさではないか？_仮説)です。本番用クラスを継承したテストクラスを作り一部メソッドをオーバーライドしてシステムの動作をテスト用に変更するような使い方は安全（プロジェクトの頓挫に繋がるような困難をもたらさない）で便利です。
設計中には[仮想メソッドを多用した抽象クラス](https://ja.wikipedia.org/wiki/Template_Method_%E3%83%91%E3%82%BF%E3%83%BC%E3%83%B3)等はうまくモデリングできているという印象を受けますが、それでも継承はプロジェクトが失敗する原因になりえます。関連：[継承構造](#継承構造)
...
[洗練された継承構造を作るべき](#問題は継承ではなく抽象的設計そのもので、しかもそれは本質的難しさではないか？_仮説)という事も考えました。

[Glb](#Glb)と継承構造でほとんどの情報処理システムを一貫して設計・認識する事ができるはずです。

つまり、Glbと継承構造によって一貫した設計をするメリットを享受し、一方で設計変更が難しくなることについては妥協すべきだろうと思います。１つのシステムを根本的な概念を変化させてまで使いまわすよりは[マイクロサービス](#１個の情報処理システムの限界)的なアイデアを考えた方が有望と思います。

# １個の情報処理システムの限界
巨大なソフトウェア開発が失敗ないし困難に直面する事が多いところを見ると、
１つの立場として、１個の情報処理システムのサイズに限界があるという考えが出てきます。

その限界を克服するには、[マイクロサービス](https://ja.wikipedia.org/wiki/%E3%83%9E%E3%82%A4%E3%82%AF%E3%83%AD%E3%82%B5%E3%83%BC%E3%83%93%E3%82%B9)のようなアイデアが有望そうです。
情報処理システムは中小規模のものが多数ある事を前提にし、それらを連携させる方法について考えるべきということです。

# 継承より構成_比較_考察
構成＝compositionとして使っています。合成と言われる事もあるようです。
私は継承ベースで設計していて、本当に継承で設計していいのか時間をかけて検討しました。
結論から言うと継承とユーザーコードによる構成はどちらも問題がありました。そして最終的には[継承構造](#継承構造)を肯定的に捉えるという事になりました。
...
他に[新たな言語機構の案](#構成的多重継承_案)について他の項目に書きました。
さらに[継承も構成も問題があるけど避けられない仮説](#問題は継承ではなく抽象的設計そのもので、しかもそれは本質的難しさではないか？_仮説)という事について書きました。
...
interface EverythingHolder extends ChildI, Parent1I, Parent2I{  
    ChildI getChild();    
    Parent1I getParetn1();  
    Parent2I getParent2();  
}  
...
それは仮想メソッドによる抽象化が無いという点で継承よりも強い相互依存です。

ここまでの苦労を払って、構成は何をもたらすか？
- p1, p2の動的変更
- 抽象クラスの上下関係を意識しなくてよくなり、横の関係になる。

継承の長所について。
継承の強すぎる制約は短所であると同時に**そのような制約が成立するというモデリング上の記述ができている**面もある。  

私は、継承が問題を起こしやすい事を理解できますが、構成が解決策だと思えません。

もし**全子孫クラスが絶対に持つべき状態**があるならそれは継承で与えるべきです。
例えば、抽象クラスにidフィールドを置いて全子孫クラスに継承させればidをメンバー変数から提供する事を促せますが、interfaceでgetId()があるというだけの制約だとその実装はもしかしたらオンラインアクセスしてどこかから状態を持ってくるかもしれないし、動作内容に前提が成立しなくなります。あるいはメンバー変数から返すにしてもtransientになっているかもしれません。そうするとシリアライズした後デシリアライズした時にidの状態が無くなります。抽象クラスはそのような問題を排除できます。  

とはいえ私は常に構成を使うべきだという人達の意見も分かります。しかし相互参照の記述が嫌です。**継承を一般的に構成に置き換えようとすると子クラスによる仮想メソッドの実装及びそれによる親クラスの動作変更を構成でどう代替するか？が問題になります。**
...
# 構成的多重継承_案
関連：[従来の継承で問題無い可能性](#問題は継承ではなく抽象的設計そのもので、しかもそれは本質的難しさではないか？_仮説)

[継承より構成_比較_考察](#継承より構成_比較_考察)の続き。

新たに多重継承専用のcompositionalクラスという概念を用意して、多重継承が何か問題を生むならそれを防止するようなアイデアをcompositionalクラスに入れたら良いだろうというアイデア。
...
2. 変数名の衝突
3. 循環継承
4. 多重継承で横の関係にある抽象クラスが相互依存するにはどうすればいいか？

1について。
CompositionalClass1$method1()のように親クラスを指定できるようにする。  
そして  
```
class ClassA extends CompositionalClass1, CompositionalClass2  
...
ClassA#equals  
```
の実装は順次親クラスのequalsを呼び出したり、その返値を活用したりする。

2について。  
名前が衝突していても全変数が存在する事とし、  
CompositionalClass1$var  
のように親クラス名を指定して区別する。
...
4について。
普通にはCompositionalClass1とCompositionalClass2は互いに依存できない。
そこで、仮想メソッドで横の関係にあるクラスの情報を取得できるようにする
```。  
//in CompositionalClass1  
abstract CompositionalClass2 getCompositionalClass2();  
//in CompositionalClass2  
abstract CompositionalClass1 getCompositionalClass1();  
```
これらメソッドをClassAに実装させる。ClassAはCompositionalClass1, 2を継承しているのでthisを返すだけで実装完了。  
```
//in ClassA  
CompositionalClass1 getCompositionalClass1(){
    return this;
}
...
```
するとCompositionalClass1とCompositionalClass2は相互に参照できる。  

その他
- compositionalクラスは直接newされず、子クラスを通してnewされる。
- メソッドの引数の型に複数のcompositionalクラスを指定できる。  
func(CompositionalClass1 & CompositionalClass2 arg1);

直列の継承と比べてどの程度意味があるか分からない。

# 構成を言語仕様でサポートする_parent_案
この案は[構成的多重継承_案](#構成的多重継承_案)の構成バージョン。

やはりcompositionalクラスという概念を作り、しかし継承ではなく構成を使い、compositionalクラスのインスタンスは「メンバー変数としての参照が1か所からに限定され、メンバー変数に代入される以外の文脈でコンストラクタが呼び出されず、その制約によって常にただ一つの親オブジェクト（自身をメンバー変数として参照するオブジェクト）が存在する事が保証され、それによってparentで親オブジェクトを参照できる」というアイデア。
...
```
public compositional class Member<T extends ParentI>{  
    public void func(){  
        parent.someMethod();//thisみたいな感じでparentが使える。  
    }  
}  
public class Parent implements ParentI{  
    private Member<ParentI> m1 = new Member<>();//m1と同じインスタンスへの参照は他にない。  
}  
```
ParentIインターフェースにMemberから利用するParentのインターフェースを記述する。こうすることでMemberとParentの相互依存が実現されている。さらにParentの他のメンバー変数を提供するインターフェースがParentIにあればそれを利用する事もできる。

# NPEとOptional
例外とtry catchを扱うJavaにおいて、nullはほとんど常に正常系の意味のある値になるはずです。
例外をcatchしたあとわざわざnullを返しているコードがあったとしても、それはthrow eを選択しなかったという点で正常系としてnullを返しています。
```
try{
...
Optionalが何やらNPEからプログラマーを守ってくれるという話がありますが、自分はこの意見に近く、ほとんど使う必要が無いと思いました。  
https://www.quora.com/Why-use-Optional-in-Java-8-instead-of-traditional-null-pointer-checks/answer/Alan-Mellor  

パイプライン的記述をする場合Optionalは役立ちますが、命令型プログラミングから外れます。  
https://devlog.atlas.jp/2019/04/24/2679  

パイプライン的記述は各処理の返値のnullが一貫して例外系を意味する事で成立します。  
例えばOptional#map()は正常系としてのnullを扱えません。nullがどこかで返ったらそれ以降指定されたマッピング関数(mapper引数）が実行されないからです。  
https://docs.oracle.com/javase/jp/8/docs/api/java/util/Optional.html#map-java.util.function.Function-  

しかしJavaでは例外が発生したなら例外を投げれるので、例外系としてのnullを無くせます。
**ですからOptional（例外系のnullがあってこそ役立ちパイプラインを書くために役立つ）というアイデアは、Javaの例外処理（nullを正常系のみにしうる）及び命令型プログラミングパラダイと矛盾するアイデアです**。
...
それは言語機構の問題ではなく、できるだけ命名規則やインターフェースの設計を統一的にする事で仕様の勘違いを減らすという問題になると思います。
...
- 失敗して良い処理ではtry catchを必ずやって、例外が生じたらcatchしてその後の処理をする。このような場合のみ例外の存在意義を感じる。
  - DBのトランザクションをやるときはtry catch。catch句ではロールバックやトランザクションの破棄等をする。基本的に重要なデータはDBに入れる（性能の問題でこの規則を守らない場合もある）。ソフトウェアの堅牢性のため、あらゆる持続的状態（ファイル、DB、グローバル変数等）はトランザクションで更新されるべきだろう。DBの更新が失敗したらロールバックする。関連：Software Transactional Memory
...
- jar hellを解決するものと良く言われていますが、jar hellを解決しません。module-info.javaの依存性の記述においてバージョンを指定しないのでjar hellを解決できる可能性はありません。
  ...
  jar hellを解決するなら[mavenとjdkを統合する](#実行時依存性解決_案)必要があるように思います。その時pom.xmlとmodule-info.javaが競合する気がします。
...
- 自己完結型のため最小の実行ファイルを作成する事に寄与します。これによってクラウド適性、コンテナ適性が高まります。[自己完結型の否定](#自己完結型の否定_批判)
...
- [アドインのためのサンドボックス](#アドインのセキュリティ)にはほとんど寄与しません。パッケージ単位での設定は粗すぎるからです。
...
- 権限付与  
  ポリシーファイルまたはSecurityManagerを継承したカスタムクラスを作りメソッドをオーバーライドする。
...
- スレッドまたはスレッドグループ単位での権限付与。できない。もしこれが出来ると通信系の処理で通信を受けて処理開始～終了までの間のスレッドの権限をメッセージ送信者に応じて設定する事でセキュアになる。
- [jar単位での権限付与](http://www.ne.jp/asahi/hishidama/home/tech/java/security.html#h_syntax)

ポリシーファイルに思った問題点。ポリシーファイルはデフォルトの位置か起動時のコマンド引数で指定した位置に置いておく必要があり、動的に与えれるものではないようだ。そのせいで、例えば起動直後にだけ特定フォルダへの書き込み権限が必要でしかしそれが終わればその権限が一切要らなくなることが分かっている場合、動作時間全体においてその権限を与え続けるしかない。タイミング別の権限付与が出来ないという事。

しかし、細かな調整はSecurityManagerを継承したクラスを作り一部メソッドをオーバーライドして対応できるようだ。
...
JavaFXを使うにしてもscene builderかFXMLかコードかで分かれる。自分はJavaFXのscene builderを少し使っていたが、直接コードで書いた方が良いと判断した。
