...
C案。
Cコードをjarに同梱し、JDKにCコンパイラを同梱し、実行時コンパイルする。Javaと相補的という意味ではCの方がC++より合う。トランスパイラ不要。学習コストが低い。コンパイル時間が短い。
- Cは静的解析ツールがあるので移植性が無いコードを排除できます。
- もし新しいコンピュータアーキテクチャを作る人が居たとしてもCコンパイラを作るのは簡単なので、JDKの実装難易度を高めません。
- Cコンパイラの普遍性
...
# 標準クラスの上書き  案
P2Pプラットフォームを作っていてp2p urlに対応させる必要が生じた。
通常のURLはJava標準のURLクラス等で扱えますが、ここでやりたいことは
URLを通じてlocalhostにあるp2pソフトウェアにアクセスするというものです。
しかもそれを自分のコードだけでなくライブラリも含めてやりたいので、
URLConnectionクラスの実装を変えたいという事になる。
だから必要なのは**クラス定義を上書きする事**。
対象クラスを継承したクラスを用意してそれで対象クラスを上書きする。
それ以降対象クラスの呼び出しはライブラリを含めてあらゆる位置から子クラスの方が使用される。
そのような事はjavassistというライブラリで出来ると思うがJVMが対応してもいいのではと思った。
実行時任意のタイミングでクラスを上書きされるとバグが出そうなので、
プロセス起動直後クラスローダーに標準クラスを上書きする段階を作るとか。
...
**コンピューターをプログラミングするという問題に対する解決のフレームワークはどんなものか**。つまりプログラミングとは根本的にどんな問題で、どんな枠組みで解決されるものなのか。仮想環境言語＋システムプログラミング言語というアイデアはその一種です。
...
あまり良い結果ではなかったようです。
C++という成熟したシステムプログラミング言語がOS開発に採用されない事、Rustという最新の注目されているシステムプログラミング言語がこの結果に終わったという事は、現時点でもOS開発でCを上回る選択肢は無く今後も無いだろうと信じる理由になります。Cコードは脆弱性が多いと言われますが、静的解析ツールである程度解決できるだろうと思います。
一方でC言語はほとんどのアプリ開発に適しません。部分的な関数をCで書くことは多々ありますが、アプリ全体をCで書くことはあまりないと思います。gitはCで書かれていますが恐らくそれはCで大規模アプリを書くという特殊スキルを伸ばした人が作ったからです。
...
**性能や大規模開発適性を求めない場合がある**。スクリプト汎用アプリ。業務上必要とされるちょっとしたツールをスクリプト言語で作りたいという需要がある。利用者数や利用頻度が限定的。
...
ではソースコードをエンドユーザーに配布して起動時にビルドするようにしたらどうかというと、JDKのように多数の言語から共通の中間コードを作成する事ができません。
...
今Wasmが必要とされている事は、最初からブラウザをJDKベースで作りJavaアプレットの高速起動を実現すべきだった事を意味しているのでは？

というのも、以下のシナリオが考えられるからだ。
**WASIがブラウザ外の仮想機械を作るならブラウザをその仮想機械上で作る事が考えられる**。
そして仮想機械を普及させて全てのアプリをwasmで作ろうとし始める。
Webのランタイムとしてセキュアなサンドボックス実行をするという特徴は変更され、何でもできるランタイムとなり、WebページのためにWasmAppletを作り権限が制限されて実行される。
...
WebページはWASI仮想環境が提供する豊富なAPIを利用したwasmアプレットを実行し、ブラウザ自体がWASI仮想環境で動作しているので起動処理が無く高速で省メモリ。**ここまで考えると、アプレットのWebページを実現するにはブラウザ自体を仮想環境で作るべきである事が分かります**。ブラウザ全体で省メモリにする、ファイルサイズを抑えるといった事を総合的に考えると、**Webページ上で実行されるアプレットと同じ仮想環境でブラウザを作った方が合理的です**。
そして、それはJDKが昔からやっているデザインです。広い範囲に利益がありそうな判断をしていったら結局JDKに到達するし、Webページ上のアプレットのためにブラウザを仮想環境ベースで実装すべき。もし本当にWebがそこに到達したら、Javaアプレットを排除した判断は間違いでブラウザがJDKベースで作られるべきだった事を自ら証明します。
そうじゃないというならwasmやWASIのターゲットは何か？どこでこのシナリオから外れる？

最も活発なエコシステムを持つJSからスタートして注目される、Webという最大のエコシステムで採用されて注目される、ブラウザ外へ進出する、GCを持たせずメモリを手動管理する言語をサポートする事で優れたベンチマークが多数発表される、高性能というイメージを持たせる事に成功したらGCオプションを実装する、ところが人々はほとんどのアプリを開発効率を理由にGC言語で書く、成熟してみればJDKや.NET Coreと変わらなかった事に気付くがその頃にはそれらは死んでいる。うまくバズっている面があるのではと思います。
...
普通ビルド処理はant等で書かれます。
私はJavaでビルド処理を書いた事がありますが簡単でした。commons-ioが便利です。
なぜGradleが必要なのか分かりません。そもそも昔からAntの必要性も良く分かりませんでした。
ビルド処理を統一的に記述したいから？統一的に記述すると可読性が高まりますが、そのプログラムと同じ言語で書いた方が可読性が高いです。
ほとんどの場合ビルド処理のコードはせいぜい50行くらいにしかなりません。長くなったとしても通常のプログラミングと大差ありません。
なぜそのために新たな言語を作るのか？
...
pom.xml + java + commons-io + maven aetherで全てのビルド処理を簡単に書けると思います。

# プログラミング言語によるビルド処理が必要になった場面　経験
ビルド直前にフォルダやファイルのコピーをしたい事は良くあります。
あるプロジェクトでsrcフォルダにシンボリックリンクを使っていましたが、JDKを別のものに変えたところ、
...
構成的多重継承が継承が抱える全ての問題を解決しないかもしれません。
例えば横の関係にあるcompositionalクラスと依存関係を作れません。
その制限はもしかしたら実際の用途において問題が無いかもしれませんが、やってみないと分かりません。
...
技術的イメージは**演出用サブ物理空間を作り、髪の毛やスカートなど一部の部位の描画を担当し、ゲームロジック用のベース物理空間と分ける**。
...
jme3のDynamicAnimControlのRagDollサンプルコードは各ボーンに物理オブジェクトを設定したりリンクを設定して物理演算している。その動作はかなり面白いが、やはりモデルの形状によってゲーム上の有利不利が生じるので対戦ゲームでは問題がある。特に使用するアバターを自由化しようとするなら、対戦ゲームにおいて当たり判定は全キャラクター共通の単純な形状であることが望ましい。
そこで、キャラクターが球体の衝突範囲を持ち、その球体の衝突箇所がそのキャラクターのサブ物理空間に通知されて、サブ物理空間で演出としてRagDoll的な動きをする、というのはアリだろう。
RagDoll的とは運動学的なモーションが自動生成される事。
