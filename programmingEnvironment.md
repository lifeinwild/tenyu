...
o.map(f).map(f).map(f)というパイプライン的記述において返値のnullは常に例外系を意味しパイプラインの次の処理が呼び出されません。

しかしJavaでは例外が発生したなら例外を投げれるので、例外系としてのnullを無くせます。
**ですからOptional（例外系のnullがあってこそ役立ちパイプラインを書くために役立つ）というアイデアは、Javaの例外処理（nullを正常系のみにしうる）及び命令型プログラミングパラダイムと矛盾するアイデアです**。
