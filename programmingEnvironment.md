...
それで[新たな言語仕様](#構成的多重継承_素案)について他の項目に記述しました。
この項目では継承も構成もダメだという事について書きます。

しかし、継承への注意喚起が行われた事は正しいと思います。
...
**継承はその多態性による利便性の分だけ設計が難しい**。再利用は良いものだと言われますが、再利用されるほど設計は熟慮が必要になり、設計の更新は難しくなります。フリーランチは存在せず何かトレードオフがあるという事です。

抽象的設計をした場合、その成果は様々なプログラムに流用できるはずです。
**本当の問題は大勢で抽象的設計を議論したり、その成果の共有が足りていない事にあるのではないか？**
...
- キーボードショートカットのみで操作できるようにしようとしたらずいぶん苦労した。
